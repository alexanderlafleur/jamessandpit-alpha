...
/**
 * This class contains all the gui stuff for the main window. Please note, that most of the event & action handling is done in de.axelwernicke.mypod.myPod.
 * 
...
public class Frontend extends javax.swing.JFrame {
    ...
    /**
     * Handle the menu selected event for the clip menu. Enables / Disables clip menu items before menu is shown
     * 
     * @param evt
     */
    ...
    /**
     * Handle the menu selected event for the playlist menu. Enables / Disables playlist menu items before menu is shown
     * 
     * @param evt
     */
    ...
    {// GEN-HEADEREND:event_playlistListViewValueChanged
        ...
        if (index >= 0 && !evt.getValueIsAdjusting()) {
            ...
            if (viewName.equals("fullListView")) {
                ...
                this.spreaderListView.setModel(myPod.getSpreaderListModel(ClipsTableUtils.ARTIST_INDEX, index));
                ...
                this.spreaderListView.setModel(myPod.getSpreaderListModel(ClipsTableUtils.ALBUM_INDEX, index));
                myPod.updateClipsTableModel(new Playlist());
            } else if (viewName.equals("genreView")) {
                // set genre view to new playlist and clear clips table
                this.spreaderListView.setModel(myPod.getSpreaderListModel(ClipsTableUtils.GENRE_INDEX, index));
                myPod.updateClipsTableModel(new Playlist());
            }
    ...
    {// GEN-HEADEREND:event_spreaderListViewValueChanged
        ...
        if (index >= 0 && !evt.getValueIsAdjusting()) {
            ...
            Vector playlistFilter = new Vector(1);
    ...
    {// GEN-HEADEREND:event_genreViewSplitPaneComponentShown
        ...
        try {
            ...
            myPod.updateClipsTableModel(new Playlist());
            this.spreaderListView.setModel(myPod.getSpreaderListModel(ClipsTableUtils.GENRE_INDEX));
    ...
    {// GEN-HEADEREND:event_albumViewSplitPaneComponentShown
        ...
        try {
            // change model of spreader list and clips table
            myPod.updateClipsTableModel(new Playlist());
            this.spreaderListView.setModel(myPod.getSpreaderListModel(ClipsTableUtils.ALBUM_INDEX));
    ...
    {// GEN-HEADEREND:event_artistViewSplitPaneComponentShown
        ...
        try {
            // change model of spreader list and clips table
            myPod.updateClipsTableModel(new Playlist());
            this.spreaderListView.setModel(myPod.getSpreaderListModel(ClipsTableUtils.ARTIST_INDEX));
    ...
    {// GEN-HEADEREND:event_clipsTableViewMouseReleased
        if (evt.isPopupTrigger()) {
            ...
            // TODO: make this customizable via preferences - play doubleclicked
            // clips
    ...
    /**
     * Gets the full list view. Since all views are sharing one listView object, there will be always the same object returned...
     * 
     ...
     * 
     * @param prefs
     *            myPod preferences to get gui properties from
     */
     ...
     * 
     * @param prefs
     *            myPod preferences object where to store gui properties
     ...
     * 
     * @param evt
     *            mouse event that triggered the popup
     */
    private void showClipsPopupMenu(java.awt.event.MouseEvent evt) {
        // make sure that the gui is not locked
        if (!this.isLocked()) {
            // select row under the mouse pointer if it is not in a selection
            // yet
            ...
            Vector playlistMenuItems = myPod.getPlaylistMenuItems();
            int playlistCount = playlistMenuItems.size();
            for (int i = 0; i < playlistCount; i++) {
                addPlaylistMenu.add((java.awt.Component) playlistMenuItems.get(i));
            }
    ...
    private void showPlaylistPopupMenu(java.awt.event.MouseEvent evt) {
        ...
        if (!this.isLocked()) {
            // get index under mouse pointer and check if the user clicked
            // within a cell
