 ...
 * It represents a single playlist. Some meta information are stored in two mhod records which this record is followed by. The first playlist item has the name of the ipod attached
 * in the mhods and contains a list of all songs. For others the name of the playlist is stored in the mhod records.
 * 
 * <PRE>
 * 
 * <BR>
...
class ITunesDBPlaylistItem {
    ...
    /**
     * Validates clip related data in a playlist. positions of a clip are set in the mhod record total song count is set correctly
     */
    void validateClips() {
        int cnt = 0;
        ITunesDBContentItem content = null;
    ...
    public String toString() {
        return new StringBuffer("[tagSize] ").append(tagSize).append('\t').append("[recordSize] ").append(recordSize).append('\t').append("[contentCount] ").append(contentCount)
                .append('\t').append("[songCount] ").append(songCount).append('\t').append("[listType] ").append(listType).toString();
    }
