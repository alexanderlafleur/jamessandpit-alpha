...
/**
 * A SongListHeader object stores the informations from the mhlt tag. It has references to the songs in the list.
 * 
 * <PRE>
 * 
 * mhlt: x00 'mhlt' - tag x04 - tag size x08 - total count of songs in the list
 * 
 * </PRE>
 * 
...
public class ITunesDBSonglistHeader {
    ...
    /**
     * Adds a Clip to iTunes DB Songlist. a new song item containing all meta data is created and added to the songlist
     * 
     * @param fileIndex
     *            of to clip in the iTunes DB
     * @param meta
     *            data of the clip
    ...
    int addClip(int fileIndex, MP3Meta meta) {
        ...
        if (meta.getTitle() != null && meta.getTitle().length() != 0) {
            songItem.attachContent(new ITunesDBContentItem(ITunesDBContentItem.TITEL, 1, ITunesDBParser.stringToUTF16LittleEndian(meta.getTitle())));
        }
