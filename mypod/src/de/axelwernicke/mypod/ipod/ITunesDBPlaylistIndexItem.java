...
/**
 * A Playlist Index object stores informations from the mhip tag. mhip: 0x0000 'mhip' - tag 0x0004 76 - tag size 0x0008 - tag size - _not_ record size ?!?! 0x000C 1 - ?? mhod
 * records that follow ?? 0x0010 0 - unknown 0x0014 0 - unknown // TODO - see dechdoc 0x0018 - song index e.g. 2002 0x0022 // TODO - see dechdoc 0x0026 // TODO - see dechdoc
 * 
...
class ITunesDBPlaylistIndexItem {
    ...
    public String toString() {
        return new StringBuffer("[tagSize] ").append(tagSize).append('\t').append("[recordSize] ").append(recordSize).append('\t').append("[contentCount] ").append(
                this.contentItems.size()).append('\t').append("[unknown5] ").append(unknown5).append('\t').append("[unknown6] ").append(unknown6).append('\t').append(
                "[songIndex] ").append(songIndex).toString();
    }
