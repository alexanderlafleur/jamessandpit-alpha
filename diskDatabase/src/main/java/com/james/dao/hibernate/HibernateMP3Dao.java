...
public class HibernateMP3Dao extends HibernateDaoSupport implements MP3Dao {
	...
	public List search(MP3SearchCriteria searchCriteria) {
		...
		if (searchCriteria.getAlbum() != null) {
			criteria.add(Restrictions.ilike("album", "%"
					+ searchCriteria.getAlbum() + "%"));
		}
		...
		if (searchCriteria.getSizeHigh() != null) {
			if (searchCriteria.getSizeLow() != null) {
				criteria.add(Restrictions.between("size", searchCriteria
						.getSizeLow(), searchCriteria.getSizeHigh()));
			} else {
				criteria.add(Restrictions.ge("size", searchCriteria
						.getSizeLow()));
			}
		} else {
			if (searchCriteria.getSizeHigh() != null) {
				criteria.add(Restrictions.le("size", searchCriteria
						.getSizeHigh()));
			} else {
				// nothing
			}
