...
public class SearchFields extends JPanel {
    ...
    public SearchFields(SearchController searchController) {
        ...
        JButton mp3SearchButton = new JButton(SearchController.SEARCH_MP3_COMMAND);
        JButton fileSearchButton = new JButton(SearchController.SEARCH_FILE_COMMAND);
    ...
    private void layoutFields() {
        ...
        predecessor = getLayoutHelper().layoutField(nameField.getLabel(), nameField.getField(), predecessor, this);

        for (Iterator i = buttons.iterator(); i.hasNext();) {
            JButton button = (JButton) i.next();

            predecessor = getLayoutHelper().layoutButton(button, predecessor, this);
