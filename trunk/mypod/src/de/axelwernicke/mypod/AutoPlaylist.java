...
public class AutoPlaylist extends Playlist implements java.io.Serializable {
    ...
    protected void update(Collection oids, de.axelwernicke.mypod.DataPool dataPool) {
        ...
        while (iter.hasNext()) {
            ...
            if (clipMeta != null
                    && !((!artistFilterEnabled || artistFilter.contains(clipMeta.getArtist()))
                            && (!albumFilterEnabled || albumFilter.contains(clipMeta.getAlbum()))
                            && (!genreFilterEnabled || genreFilter.contains(clipMeta.getGenre())) && (!yearFilterEnabled || yearFilter
                            .contains(new Integer(clipMeta.getYear()))))) {
                this.remove(clipOid);
            }
        }
        ...
        while (iter.hasNext()) {
            ...
            if (clipMeta != null && !this.containsClip(clipOid) && (!artistFilterEnabled || artistFilter.contains(clipMeta.getArtist()))
                    && (!albumFilterEnabled || albumFilter.contains(clipMeta.getAlbum()))
                    && (!genreFilterEnabled || genreFilter.contains(clipMeta.getGenre()))
