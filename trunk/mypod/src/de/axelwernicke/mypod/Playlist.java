...
/**
 * This class represents a playlist within myPod. A playlist is a vector of clip
 * oid's plus some further information like a name, the total playtime...
 * 
...
public class Playlist implements java.io.Serializable {
     ...
     *            of the new playlist
     * @param oids
     *            list of clips to put into the new playlist
     */
    public Playlist(String name, Vector oids) {
        this.name = name;
        list = new Vector(oids.size());
        this.addClips(oids);
    }
     ...
     * 
     * @param oids
     *            of the clips to remove from the playlist
     */
    public void remove(Vector oids) {
        Iterator iter = oids.iterator();
        while (iter.hasNext()) {
            remove((Long) iter.next());
        }
    }

    /**
     * Removes an item from the playlist, and corrects the total time of the
     * playlist.
     * 
     * @param oid
    ...
    public void remove(Long oid) {
        ...
        while (oid != null && list.contains(oid)) {
            ...
            if (mp3meta != null) {
                ...
                totalFilesize -= mp3meta.getFilesize();
            }
     ...
     * 
     * @param totalTime
     *            New value of property totalTime.
     */
     ...
     * 
     * @param oid
     *            clip to search for in the playlist
    ...
    public void addClip(Long oid) {
        ...
        if (oid != null && !list.contains(oid)) {
            ...
            totalFilesize += myPod.getBackend().getDataPool().getMeta(oid).getFilesize();
        }
    }
     ...
     *            of the clips to add
     */
    public void addClips(Vector oids) {
        if (oids == null) {
            return;
        }
        ...
        for (int i = 0; i < oidsSize; i++) {
            ...
            if (!list.contains(oid)) {
                ...
                totalFilesize += dataPool.getMeta(oid).getFilesize();
            }
        }
    }
     ...
     * 
     * @param value
     *            true to enable synchronization to iPod
     */
     ...
     * 
     * @param dataPool
     *            to determine artists for clips
     * @return vector containing all artists
     */
    Vector getAllArtists(DataPool dataPool) {
        Vector result = new Vector(101);

        Iterator clipIter = list.iterator();

        while (clipIter.hasNext()) {
            String artist = dataPool.getMeta((Long) clipIter.next()).getArtist();

            if (!result.contains(artist)) {
                result.addElement(artist);
            }
        }
        // sort artists
     ...
     * 
     * @return vector containing the oids of all clips
     */
    public Vector getAllClips() {
        return (Vector) list.clone();
    }

    /**
     * Gets path and filename for all clips in the playlist as '"name1"
     * "name2"'.
     * 
     * @param dataPool
     *            to determine path and filename from.
    ...
    String getFilesString(DataPool dataPool) {
        ...
        while (iter.hasNext()) {
            buffer.append("\"");
            buffer.append(dataPool.getMeta((Long) iter.next()).getFilePath());
            buffer.append("\" ");
        }
    ...
    public void validateTotalFilesize() {
        ...
        while (iter.hasNext()) {
            ...
            if (oid != null && meta != null) {
                newTotalFilesize += meta.getFilesize();
            }
        }
     ...
     * 
     * @param totalFilesize
     *            New value of property totalFilesize.
     */
