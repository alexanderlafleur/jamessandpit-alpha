...
public class Backend {
    ...
    /**
     * Creates a new instance of Backend <BR>- deserialize preferences <BR>- initialize datapool & ipod <BR>- deserialize playlists <BR>- validate playlists <BR>- initialize
    ...
    /**
     * shuts the backend down. <BR>- serialzie myPod preferences <BR>- serialize playlists <BR>- shut down the data pool
     */
    ...
    /**
     * Applies the filters to an autoplaylist <BR>- get the playlist <BR>- check if we have an autoplaylist <BR>- get all enabled filters <BR>- remove clips from the playlist
     * if they not apply to the filter (anymore) <BR>- find all clips that apply to the filter <BR>- add found clips, if they are not in the playlist yet <BR>- update playtime
     * for the playlist
     * 
     * @param index
     *            index of the playlist in the list of playlists
    ...
    /**
     * Update all autoplaylists <BR>- iterate over all playlists and update the autoplaylists <BR>- additionally total time and total filesize are validated
     */
    ...
    /**
     * Validates all playlists. Each entry in each playlists is checked. If the entry (oid) is not contained in the data pool, its removed from the playlist. Attention: This is
    ...
    /**
     * Scans a list of files and directories for mp3 files. <BR>- get all selected files and all files in selected direcories <BR>- if file is not known yet, scan file <BR>- if
     * not, check if the file was touched since last scan, if so, rescan and update meta data
     * 
     * @param dialog
    ...
    public int scanFiles(File[] selection, de.axelwernicke.mypod.gui.ScanDialog dialog) {
        ...
        for (File element : selection) {
            currFile = (File) element;
            if (currFile != null) {
        ...
        while (!files.isEmpty()) {
            ...
            // do some gui stuff
            dialog.statusContentLabel.setText(new StringBuffer(GuiUtils.getStringLocalized("scanningFiles")).append(" (").append(totalScannedCnt).append("/").append(fileCnt)
                    .append(")").toString());
    ...
    /**
     * deserializes all playlists <BR>- check if file exists <BR>- heck if file is writable, if not wait and retry <BR>- deserialize vector of playlists <BR>- copy playlists
     * into a default list model
     * 
     * @return success
     */
    ...
    private boolean serializePreferences() {
        ...
        File preferencesFile = new File(PREFERENCES_GZ_FILENAME);
    ...
    /**
     * Deserializes myPods preferences <BR>- check if the file exists <BR>- check if the file is writable <BR>- load and decode preferences file
     * 
     * @return success
     */
    ...
    /**
     * creates a new playlist from an m3u file. <BR>- determine name of the playlist from filename
     * 
     * @param file
     *            m3u playlist
    ...
    /**
     * Saves a playlist as m3u file. <br>- make sure, that the filename ends to .m3u <br>- create file <br>- write file header <br>- write filter information for autoplaylists
     * <br>-
     * 
     * @param filename
     *            of the playlist
    ...
    public void savePlaylistM3U(String filename, Playlist playlist) {
        try {
            ...
            while (oidIter.hasNext()) {
                ...
                fos.write(new StringBuffer("#EXTINF:").append(meta.getDuration()).append(",").append(meta.getArtist()).append(" - ").append(meta.getTitle()).append("\n")
                        .toString().getBytes());
            ...
            JOptionPane.showMessageDialog(null, GuiUtils.getStringLocalized("resource/language", "couldNotWritePlaylistTo") + ": \n" + filename + "\n"
                    + GuiUtils.getStringLocalized("makeSureAllDirsExistAndYouAreAllowedToWriteInto") + ".", GuiUtils.getStringLocalized("anErrorOccured"),
    ...
    /**
     * Removes a clip from myPod First the clip is removed from all playlist, the from data pool.
     * 
     * @param oid
    ...
    public void serializeIPodMapper(Hashtable mapper) {
        ...
        try {
            XMLEncoder xe = new XMLEncoder(new BufferedOutputStream(new FileOutputStream(IPOD_MAPPER_FILENAME)));
    ...
    private Hashtable deserializeIPodMapper() {
        ...
        try {
            XMLDecoder xd = new XMLDecoder(new BufferedInputStream(new FileInputStream(IPOD_MAPPER_FILENAME)));
    ...
    /**
     * Synchronizes myPod playlists with iPod. <BR>- load iTunes database from iPod <BR>- load iMapper <BR>- determine all clips to synchronize <BR>- determine clips to remove
     * from iPod <BR>- determine clips to move to iPod <BR>- check space left on iPod <BR>- remove clips from iPod <BR>- remove empty playlists from iPod <BR>- copy clips to
     * iPod <BR>- create playlists on iPod <BR>- save iTunes database to iPod <BR>- serialize iMapper
     * 
     * @param dialog
     *            to show progress
    ...
    /**
     * check clips to remove from iPod ->clips are to be removed if clip is on the iPod but not on a playlist to synchronize
     * 
     * @param clipsToSync
    ...
    /**
     * Gets clips to move to iPod ->clips are moved if clip is on a playlist, but not on the iPod
     * 
     * @param syncOids
    ...
    /**
     * Gets the total size of all clips known by myPods data pool - get a list of oids for all clips <BR>- summarize the size of each clip
     * 
    ...
    /**
     * Gets the total duration of all clips known by myPods data pool - get a list of oids for all clips <BR>- summarize the duration of each clip
     * 
    ...
    /**
     * Sends a playlist to an external player <BR>- determine parameter (add or append) <BR>- check that tmp directory exists to store playlist in <BR>- check that configured
     * player can be found <BR>- store playlist <BR>- call player
     * 
     * @param playlist
     *            to play
     * @param append
            ...
            try {
                ...
                if (!new File(playerPath).exists()) {
                    JOptionPane.showMessageDialog(null, GuiUtils.getStringLocalized("couldNotFindExternalPlayer") + ": \n" + playerPath, GuiUtils
                            .getStringLocalized("checkConfiguration"), JOptionPane.ERROR_MESSAGE);
                    return;
                }
     ...
     */
    public void reorganizeClips(de.axelwernicke.mypod.gui.ReorganizeClipsProgressDialog dialog, String baseDirectory, int dirStructureIndex, int filenameStructureIndex,
     ...
     */
    public void setClipsProperties(ProgressDialog dialog, MP3Meta newMeta, Vector oids) {
        MP3Meta origMeta;
        Long oid;
        ...
        for (int cnt = 0; cnt < clipCnt; cnt++) {
            try {
                ...
                if (origMeta != null) {
                    ...
                    getDataPool().updateData(oid, ID3Utils.scanMp3VdHeide(new File(origMeta.getFilePath())));
                }
            } catch (Exception e) {
                logger.warning("Exception raised: " + e.getMessage());
                e.printStackTrace();
        ...
        while (clipsIter.hasNext()) {
            try {
                ...
                if (!found) {
                    ...
                    // skip or abort
                    int result = JOptionPane.showInternalConfirmDialog(null, GuiUtils.getStringLocalized("doYouWantToCancelTheAction"), GuiUtils
                            .getStringLocalized("pleaseConfirm"), JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
