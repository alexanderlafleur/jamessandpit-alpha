...
public class Backend {
	...
	private static final String PLAYLISTS_GZ_FILENAME = "data" + File.separator
			+ "playlists.xml.gz";

	/** name of the file to backup the playlistfile to */
	private static final String PLAYLISTS_GZ_BACKUP_FILENAME = "data"
			+ File.separator + "playlists.xml.gz.bak";

	/** name of the file all playlists are serialized to */
	private static final String PLAYLISTS_FILENAME = "data" + File.separator
			+ "playlists.xml";

	/** name of myPods preferences file */
	private static final String PREFERENCES_GZ_FILENAME = "data"
			+ File.separator + "preferences.xml.gz";

	/** name of the file the playlist file is backed up to */
	private static final String PREFERENCES_GZ_BACKUP_FILENAME = "data"
			+ File.separator + "preferences.xml.bak.gz";

	/** name of the file all playlists are serialized to */
	private static final String PREFERENCES_FILENAME = "data" + File.separator
			+ "preferences.xml";
	...
	/**
	 * model for the spreader - a list of all artists / albums / genres of a
	 * playlist
	 */
	...
	/**
	 * Creates a new instance of Backend <BR>- deserialize preferences <BR>-
	 * initialize datapool & ipod <BR>- deserialize playlists <BR>- validate
	 * playlists <BR>- initialize list view table model
	 */
	public Backend() {
		...
		if (this.playlistListModel.size() == 0) {
			playlist = new Playlist();
		} else {
			playlist = (Playlist) this.playlistListModel.getElementAt(0);
		}
		// init list view model from first playlist (which always exist...)
		clipsListTableModel = new ClipsTableModel(playlist);

		logger.exiting("Backend", "Backend");
	}

	/**
	 * shuts the backend down. <BR>- serialzie myPod preferences <BR>-
	 * serialize playlists <BR>- shut down the data pool
	 */
	 ...
	 * 
	 * @param columnCode
	 *            for the column to get a model for
	 * @param playlistIdx
	 *            index of playlist to collect list items from
	 * @return list model containing a distinct selection of the specified
	 *         column, ordered
	 */
	protected DefaultListModel getSpreaderListModel(int columnCode,
			int playlistIdx) {
		// get the content for the selected column
		Vector content;
		switch (columnCode) {
		case ClipsTableUtils.ARTIST_INDEX:
			content = this.getDataPool().getAllArtistValues(
					this.getPlaylist(playlistIdx).getAllClips(), true);
			break;
		case ClipsTableUtils.ALBUM_INDEX:
			content = this.getDataPool().getAllAlbumValues(
					this.getPlaylist(playlistIdx).getAllClips(), true);
			break;
		case ClipsTableUtils.GENRE_INDEX:
			content = this.getDataPool().getAllGenreValues(
					this.getPlaylist(playlistIdx).getAllClips(), true);
			break;
		default:
	 ...
	 * 
	 * @param index
	 *            index of the playlist to delete in the list of playlists
	 */
	...
	public Playlist getPlaylist(String name) {
		...
		Playlist currList;
		boolean found = false;
		for (Enumeration playlistIter = this.getPlaylistList().elements(); !found
				&& playlistIter.hasMoreElements();) {
	...
	/**
	 * Applies the filters to an autoplaylist <BR>- get the playlist <BR>-
	 * check if we have an autoplaylist <BR>- get all enabled filters <BR>-
	 * remove clips from the playlist if they not apply to the filter (anymore)
	 * <BR>- find all clips that apply to the filter <BR>- add found clips, if
	 * they are not in the playlist yet <BR>- update playtime for the playlist
	 * 
	 * @param index
	 *            index of the playlist in the list of playlists
	 */
	...
	/**
	 * Update all autoplaylists <BR>- iterate over all playlists and update the
	 * autoplaylists <BR>- additionally total time and total filesize are
	 * validated
	 */
	...
	/**
	 * Validates all playlists. Each entry in each playlists is checked. If the
	 * entry (oid) is not contained in the data pool, its removed from the
	 * playlist. Attention: This is pretty time consuming!
	 */
	public void validatePlaylists() {
		...
		for (int i = 0; i < playlistCnt; i++) {
			...
			if (playlist != null) {
				// check that there is no key ( oid ) without meta data
				// object...
				for (int idx = oids.size() - 1; idx >= 0; idx--) {
					oid = (Long) oids.get(idx);
					if (oid == null || getDataPool().getMeta(oid) == null) {
						logger.warning("oid " + oid
								+ " was invalid, removing entry");
			...
			if (invalidCnt > 0) {
				logger.info("checking playlist " + playlist.getName()
						+ " resulted in " + validCnt + " , but " + invalidCnt
						+ " invalid clips.");
			}
		} // for all playlists
	...
	public long getTotalFilesize(Vector oids) {
		...
		Iterator oidIter = oids.iterator();
		while (oidIter.hasNext()) {
			totalFileSize += dataPool.getMeta((Long) oidIter.next())
					.getFilesize();
		}
	...
	/**
	 * Scans a list of files and directories for mp3 files. <BR>- get all
	 * selected files and all files in selected direcories <BR>- if file is not
	 * known yet, scan file <BR>- if not, check if the file was touched since
	 * last scan, if so, rescan and update meta data
	 * 
	 * @param dialog
	 ...
	 */
	public int scanFiles(File[] selection,
			de.axelwernicke.mypod.gui.ScanDialog dialog) {
		...
		// do some gui stuff
		dialog.statusContentLabel.setText(GuiUtils
				.getStringLocalized("determiningFilesToScan"));
		dialog.totalProgressBar.setIndeterminate(true);
		dialog.cancelButton.setEnabled(false);

		// get all files to scan
		for (int i = 0; i < selection.length; i++) {
			currFile = (File) selection[i];
			if (currFile != null) {
				if (currFile.isDirectory()) {
					files.addAll(FileUtils.getAllFiles(currFile,
							FileUtils.mp3FilePattern));
				} else {
					files.add(currFile);
				}
			}
		...
		while (!files.isEmpty()) {
			...
			// get file from the end of the vector
			currFile = (File) files.remove(files.size() - 1);

			// do some gui stuff
			dialog.statusContentLabel.setText(new StringBuffer(GuiUtils
					.getStringLocalized("scanningFiles")).append(" (").append(
					totalScannedCnt).append("/").append(fileCnt).append(")")
					.toString());
			...
			if (dataPool.isInPool(currFile)) {
				...
				if (currFile.lastModified() != dataPool
						.getLastModified(currFile)) {
	...
	private void serializePlaylists() {
		...
		if (playlistFile.exists()) {
			FileUtils
					.copy(playlistFile, new File(PLAYLISTS_GZ_BACKUP_FILENAME));
		}

		// since jdk 1.4 has a problem to serialize javax.swing.DefaultListModel
		// to xml, lets copy the content in
		// a simple Vector
		...
		try {
			XMLEncoder xe = new XMLEncoder(new GZIPOutputStream(
					new FileOutputStream(playlistFile), 65535));
	...
	/**
	 * deserializes all playlists <BR>- check if file exists <BR>- heck if
	 * file is writable, if not wait and retry <BR>- deserialize vector of
	 * playlists <BR>- copy playlists into a default list model
	 * 
	 * @return success
	 */
	private boolean deserializePlaylists() {
		...
		try {
			...
			if (FileUtils.isWritable(playlistFile, 100000)) {
				...
				XMLDecoder xd = new XMLDecoder(new GZIPInputStream(
						new FileInputStream(playlistFile), 65535));
	...
	/**
	 * serializes all preferences <BR>- encode and save prefs to file
	 * 
	 * @return success
	 */
	private boolean serializePreferences() {
		...
		File preferencesFile = new File(this.PREFERENCES_GZ_FILENAME);

		if (preferencesFile.exists()) {
			FileUtils.copy(preferencesFile, new File(
					PREFERENCES_GZ_BACKUP_FILENAME));
		}

		try {
			XMLEncoder xe = new XMLEncoder(new GZIPOutputStream(
					new FileOutputStream(preferencesFile), 65535));
	...
	/**
	 * Deserializes myPods preferences <BR>- check if the file exists <BR>-
	 * check if the file is writable <BR>- load and decode preferences file
	 * 
	 * @return success
	 */
	private boolean deserializePreferences() {
		...
		try {
			...
			if (FileUtils.isWritable(prefsFile, 100000)) {
				...
				XMLDecoder xd = new XMLDecoder(new GZIPInputStream(
						new FileInputStream(prefsFile), 65535));
	...
	/**
	 * creates a new playlist from an m3u file. <BR>- determine name of the
	 * playlist from filename
	 * 
	 * @param file
	 *            m3u playlist
	...
	public Playlist loadPlaylistM3U(File file) {
		...
		try {
				...
				try {
					...
					if (!line.startsWith("#")) {
						...
						if (oid != null) {
							...
							logger.info("unable to find clip " + line
									+ " in the data pool.");
						}
					} else {
						...
						if (line.startsWith("#mypod artist")) {
							...
							yearFilter.add(Integer.valueOf(line.substring(11)
									.trim()));
						}
					}
			...
			if (artistFilter.size() > 0 || genreFilter.size() > 0
					|| yearFilter.size() > 0) {
	...
	/**
	 * Saves a playlist as m3u file. <br>- make sure, that the filename ends to
	 * .m3u <br>- create file <br>- write file header <br>- write filter
	 * information for autoplaylists <br>-
	 * 
	 * @param filename
	 *            of the playlist
	 * @param playlist
	...
	public void savePlaylistM3U(String filename, Playlist playlist) {
		try {
			...
			while (oidIter.hasNext()) {
				...
				fos.write(new StringBuffer("#EXTINF:").append(
						meta.getDuration()).append(",")
						.append(meta.getArtist()).append(" - ").append(
								meta.getTitle()).append("\n").toString()
						.getBytes());
			...
			JOptionPane
					.showMessageDialog(
							null,
							GuiUtils.getStringLocalized("resource/language",
									"couldNotWritePlaylistTo")
									+ ": \n"
									+ filename
									+ "\n"
									+ GuiUtils
											.getStringLocalized("makeSureAllDirsExistAndYouAreAllowedToWriteInto")
									+ ".", GuiUtils
									.getStringLocalized("anErrorOccured"),
	...
	/**
	 * Removes clips from myPod First the clip is removed from all playlist, the
	 * from data pool.
	 * 
	 * @param oids
	 *            of the clips to delete
	 */
	...
	/**
	 * Removes a clip from myPod First the clip is removed from all playlist,
	 * the from data pool.
	 * 
	 * @param oid
	 *            of the clip to remove
	 */
	private void removeClipFromMyPod(Long oid) {
		...
		// remove clip from all playlists - DO THIS BEFORE REMOVING THE CLIP
		// FROM DATA POOL
		...
		// if we are in an spreader view, the playlist displayed must be handled
		// here ...
		if (!this.playlistListModel.contains(this.clipsListTableModel
				.getPlaylist())
	...
	public void deleteClips(Vector oids) {
		...
		while (iter.hasNext()) {
			...
			deleted = FileUtils.delete(new File(this.dataPool.getMeta(oid)
					.getFilePath()));
	 ...
	 * 
	 * @param playlist
	 *            to remove clips from
	 * @param oids
	 *            of the clips to remove
	 */
	...
	/**
	 * Gets all clips that are missing. all clips in the data pool are checked
	 * for their files. If the file can not be opended, the oid is added to the
	 * vector of missing clips
	 * 
	 * @return vector containing oids of missing clips or null, if action was
	 *         canceled
	 * @param dialog
	...
	public Vector scanForMissingMediaFiles(ProgressDialog dialog) {
		...
		// do some gui stuff
		dialog.setStatusText(GuiUtils
				.getStringLocalized("searchingForMissingMediaFiles"));
	...
	public void serializeIPodMapper(Hashtable mapper) {
		...
		try {
			XMLEncoder xe = new XMLEncoder(new BufferedOutputStream(
					new FileOutputStream(this.IPOD_MAPPER_FILENAME)));
	...
	private Hashtable deserializeIPodMapper() {
		...
		try {
			XMLDecoder xd = new XMLDecoder(new BufferedInputStream(
					new FileInputStream(this.IPOD_MAPPER_FILENAME)));
	...
	/**
	 * Synchronizes myPod playlists with iPod. <BR>- load iTunes database from
	 * iPod <BR>- load iMapper <BR>- determine all clips to synchronize <BR>-
	 * determine clips to remove from iPod <BR>- determine clips to move to
	 * iPod <BR>- check space left on iPod <BR>- remove clips from iPod <BR>-
	 * remove empty playlists from iPod <BR>- copy clips to iPod <BR>- create
	 * playlists on iPod <BR>- save iTunes database to iPod <BR>- serialize
	 * iMapper
	 * 
	 * @param dialog
	 *            to show progress
	...
	public long synchronizeIPod(de.axelwernicke.mypod.gui.IPodSyncDialog dialog) {
		...
		dialog.totalProgressBar.setIndeterminate(true);
		dialog.statusContentLabel.setText(GuiUtils
				.getStringLocalized("loadingDababase"));
		iPod.getITunesDB();
		dialog.totalProgressBar.setIndeterminate(false);

		// deserialize iPod Mapper, this hashmap contains oids as key and iPod
		// fileindizes as content.
		Hashtable iMapper = deserializeIPodMapper();

		// set dialog
		dialog.statusContentLabel.setText(GuiUtils
				.getStringLocalized("determiningFilesToSync"));
		...
		if (spaceLeft > 0) {
			// update dialog
			dialog.statusContentLabel.setText(GuiUtils
					.getStringLocalized("removingClips"));
			...
			// copy clips to iPod
			dialog.statusContentLabel.setText(GuiUtils
					.getStringLocalized("copyingFiles"));
	 ...
	 * 
	 * @param clipsToSync
	 *            vector of oids - clips to synchronize
	 * @param iMapper
	 *            mapping from oids to iPod song id
	 * @return fileIndex of the clips to remove
	 */
	private Vector getClipsToRemoveFromIPod(Vector clipsToSync,
			Hashtable iMapper) {
		...
		logger.fine("found " + clips2Remove.size()
				+ " clips to remove from iPod");
	 ...
	 * 
	 * @param syncOids
	 *            oids of the clips to synchronize
	 * @param iMapper
	 *            hashtable that defines which songs are synchronized yet
	...
	/**
	 * Gets the total count of clips known by myPods data pool - get the size of
	 * a list of oids for all clips
	 * 
	 ...
	 * of oids for all clips <BR>- summarize the size of each clip
	 * 
	 ...
	 * list of oids for all clips <BR>- summarize the duration of each clip
	 * 
	 ...
	 * size from local db via oid. The oid _must_ be known by myPod!
	 * 
	 * @param oids
	...
	/**
	 * Sends a playlist to an external player <BR>- determine parameter (add or
	 * append) <BR>- check that tmp directory exists to store playlist in <BR>-
	 * check that configured player can be found <BR>- store playlist <BR>-
	 * call player
	 * 
	 * @param playlist
	 *            to play
	 * @param append
	 *            if true, playlist is appended to the clips on the player
	 */
	public void playExtern(Playlist playlist, boolean append) {
		if (playlist != null) {
			try {
				String parameter = (append) ? preferences.getAppendParameter()
						: preferences.getAddParameter();
				String fs = File.separator;

				// check external player
				String playerPath = preferences.getPlayerPath();
				if (!new File(playerPath).exists()) {
					JOptionPane.showMessageDialog(null, GuiUtils
							.getStringLocalized("couldNotFindExternalPlayer")
							+ ": \n" + playerPath, GuiUtils
							.getStringLocalized("checkConfiguration"),
							JOptionPane.ERROR_MESSAGE);
					return;
				}

				// create tmp playlist
				String playlistName = System.getProperty("user.dir") + fs
						+ "tmp" + fs + "play.m3u";
				savePlaylistM3U(playlistName, playlist);

				// create and execute command
				String command = "\"" + playerPath + "\" " + parameter + " \""
						+ playlistName + "\"";
				java.lang.Runtime.getRuntime().exec(command);
			} catch (Exception e) {
				logger
						.warning("Exception occured while trying to start external player :"
								+ e.getMessage());
				e.printStackTrace();
			}
	 ...
	 * 
	 * @param baseDirectory
	 *            to determine clips below
	 * @return vector of oids
	 */
	private Vector getClipsBelowDirectory(String baseDirectory) {
		Vector oids = new Vector();
		Long oid;
		for (Iterator oidIter = this.getDataPool().getAllOid().iterator(); oidIter
				.hasNext();) {
			oid = (Long) oidIter.next();
			if (getDataPool().getMeta(oid).getFilePath().startsWith(
	...
	/**
	 * Reorganizes filestructure and renames files based on id3 tags <BR>
	 * determine clips that are below base directory <BR>
	 * for all clips to reorganize: determine new pathname <BR>
	 * for all clips to reorganize: determine new filename <BR>
	 * for all clips to reorganize: create new file <BR>
	 * 
	 * @param dialog
	 *            reorganizing progress dialog
	 * @param removeEmptyDirectories
	 *            flag, if true - empty directories are removed
	 * @param baseDirectory
	 *            all files below are reorganized
	 * @param dirStructureIndex
	 *            index that determines how the filestructure is reorganized
	 * @param filenameStructureIndex
	 *            index that determines how the files are renamed
	 */
	public void reorganizeClips(
			de.axelwernicke.mypod.gui.ReorganizeClipsProgressDialog dialog,
			String baseDirectory, int dirStructureIndex,
			int filenameStructureIndex, boolean removeEmptyDirectories) {
		...
		// do some gui stuff
		dialog.statusContentLabel.setText(GuiUtils
				.getStringLocalized("determiningClipsToReorganize"));
		dialog.totalProgressBar.setIndeterminate(true);
		...
		for (int i = 0; i < size; i++) {
			...
			// get valid meta data values to create new directory and filenames
			// from
			...
			// do some gui stuff
			dialog.statusContentLabel.setText(GuiUtils
					.getStringLocalized("reorganizingClips")
					+ " (" + i + "/" + oids.size() + ")");
			...
			dialog.clipsRenamedContentLabel.setText(String
					.valueOf(filesRenamed));
			dialog.directoriesRemovedContentLabel.setText(String.valueOf(0));

			try {
				// determine new directory name
				oldDirectoryName = meta.getFilePath().substring(0,
						meta.getFilePath().lastIndexOf(File.separator));
				...
				case 1:
					...
					break;
				default:
					newFilename = meta.getFilename();
					break;
				}

				// create new file
				newFile = new File(newDirectoryName + File.separator
						+ newFilename);

				// check if directory change
				dirChanged = !(new File(newDirectoryName).equals(new File(
						oldDirectoryName)));
				if (dirChanged) {
					filesMoved++;

					logger.finer("dir changed from " + oldDirectoryName
							+ " to " + newDirectoryName);
					...
					if (!newDir.exists() || !newDir.canWrite()) {
						// if the dir is not there or not readable yet, we are
						// in serious trouble...
						throw new Exception("could not get directory " + newDir
								+ " writable");
					}
				}

				// check if filename changed
				filenameChanged = !(new File(newFilename).equals(new File(meta
						.getFilename())));
				if (filenameChanged) {
					filesRenamed++;

					logger.finer("filename changed from " + meta.getFilename()
							+ " to " + newFilename);

					// check if the file exists, change filename if so
					if (newFile.exists()) {
						String tmpFileName = newFilename.substring(0,
								newFilename.lastIndexOf("."));
						int cnt = 0;
						do {
							// check file exteded by counter
							newFile = new File(newDirectoryName
									+ File.separator + tmpFileName + " (" + cnt
									+ ").mp3");
							cnt++;
						} while (newFile.exists() && cnt < 100000);
					}

					if (newFile.exists()) // if the dir is not there or not
					// readable yet, we are in serious
					// trouble...
					{
						throw new Exception(
								"could not get new filename last try was: "
										+ newFile);
					}
				}
				...
				if (dirChanged || filenameChanged) {
					boolean success = FileUtils.move(new File(meta
							.getFilePath()), newFile);
		...
		if (removeEmptyDirectories) {
			...
			// do some gui stuff
			dialog.statusContentLabel.setText(GuiUtils
					.getStringLocalized("determiningDirectoriesToRemove"));
			dialog.totalProgressBar.setIndeterminate(true);
			...
			do {
				...
				// update gui
				dialog.directoriesRemovedContentLabel.setText(String
						.valueOf(totalDirsDeleted));
			...
			logger.info("" + totalDirsDeleted + " directories deleted in "
					+ cnt + " loops");
		}
	 ...
	 * @param newMeta
	 *            mp3 meta data to set
	 * @param oids
	 *            of the clips to update id3 tags
	 */
	public void setClipsProperties(ProgressDialog dialog, MP3Meta newMeta,
			Vector oids) {
		MP3Meta origMeta;
		Long oid;
		...
		for (int cnt = 0; cnt < clipCnt; cnt++) {
			try {
				oid = (Long) oids.get(cnt);
				origMeta = dataPool.getMeta(oid);

				if (origMeta != null) {
					// update gui
					dialog.setStatusText(GuiUtils
							.getStringLocalized("updatingID3Tags")
							+ " ( " + (cnt + 1) + " / " + clipCnt + " )");
					dialog.setProgressValue(cnt);
					dialog.setClipText(origMeta.getFilename());

					// do tag updating
					ID3Utils.updateMp3VdHeide(origMeta, newMeta);

					// update clip from file
					// TODO if we can be sure that tag writing works, we should
					// avoid scanning and set data directly
					getDataPool().updateData(
							oid,
							ID3Utils.scanMp3VdHeide(new File(origMeta
									.getFilePath())));
				}
			} catch (Exception e) {
				logger.warning("Exception raised: " + e.getMessage());
	...
	/**
	 * Relocates clips if their media files where moved. e.g:
	 * a:\bbb\ccc\ddd\fool.mp3 --> k:\lll\mmm\nnn\ddd\fool.mp3 - baseSrc - | --
	 * file -- -- baseDest -- | -- file --
	 * 
	 * @param dialog
	 *            to show progress
			...
			try {
				...
				while (!found && movesIter.hasMoreElements()) {
					...
					if (meta.getFilePath().startsWith(recentMoveSrc)) {
						// build potential new location and check if the new
						// location exists
						newLocation = moves.get(recentMoveSrc)
								+ oldLocation.substring(recentMoveSrc.length());
						if ((new File(newLocation)).exists()) {
							found = true;
						}

						logger.fine("Tried " + newLocation
								+ " as new location (recent move), result : "
								+ found);
					}
				...
				if (!found) {
					...
					fc.setDialogTitle(GuiUtils
							.getStringLocalized("findNewLocationFor")
							+ " " + oldLocation);
					...
					if (res == JFileChooser.APPROVE_OPTION) {
						...
						logger
								.fine("Tried "
										+ newLocation
										+ " as new location (user selection), result : "
										+ found);
					}
				}

				if (!found) {
					// if still not found the user gave us nonsense ... ask for
					// skip or abort
					int result = JOptionPane
							.showInternalConfirmDialog(
									null,
									GuiUtils
											.getStringLocalized("doYouWantToCancelTheAction"),
									GuiUtils
											.getStringLocalized("pleaseConfirm"),
					...
					// first we need to know how far meta.getFilename and
					// newLocation equal _from the end_ (!!)
					int oldLength = oldLocation.length();
					int newLength = newLocation.length();
					int minLen = (oldLength < newLength) ? oldLength
							: newLength;
					int equalChars = 0;
					boolean equal;

					do {
						equal = (oldLocation.charAt(oldLength - 1 - equalChars) == newLocation
								.charAt(newLength - 1 - equalChars));
					} while (equal && equalChars++ < minLen);

					// extract and store base path
					moves.put(oldLocation.substring(0, oldLength - equalChars),
							newLocation.substring(0, newLength - equalChars));

					logger.fine("put new move: "
							+ oldLocation.substring(0, oldLength - equalChars)
							+ " : "
							+ newLocation.substring(0, newLength - equalChars));
