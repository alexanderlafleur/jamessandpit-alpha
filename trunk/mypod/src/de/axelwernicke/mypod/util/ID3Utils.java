...
public class ID3Utils {
    ...
    /**
     * Creates a new instance of ID3Utils. The constructor is private, cause
     * this class is singleton. Please use getInstance() instead.
     */
    private ID3Utils() {
    }
    ...
    /**
     * Scans an mp3 file for id3 tags. <BR>- extract id3 tags <BR>- return
     * created MP3Meta object
     * 
     * @return mp3 meta data of the file
     * @param mp3File
     ...
     * @param metaToSet
     *            meta data to set, values are set if they are not null
     *            (strings) or -1 (numbers)
     */
    public static void updateMp3VdHeide(MP3Meta origMeta, MP3Meta metaToSet) {
        ...
        try {
            ...
            try { // set new cdIdentifier
                tag = createTag((origMeta.getCdIdentifier() != null) ? origMeta.getCdIdentifier().getBytes() : "".getBytes(), (metaToSet
                        .getCdIdentifier() != null) ? metaToSet.getCdIdentifier().getBytes() : "".getBytes());
                if (tag != null) {
            ...
            try { // set new playcounter
                tag = createTag((origMeta.getPlayCounter() != null) ? origMeta.getPlayCounter().getBytes() : "".getBytes(), (metaToSet
                        .getPlayCounter() != null) ? metaToSet.getPlayCounter().getBytes() : "".getBytes());
                if (tag != null) {
            ...
            logger
                    .warning("Exception raised for "
                            + ((mp3File != null) ? mp3File.getName() : "unknown file (file == null)" + " : " + e.getMessage()));
            e.printStackTrace();
        }
    ...
    /**
     * Prepares an mp3 file to be updated. All tags are read once and id3v1 tags
     * are written
     * 
     * @param mp3File
     *            to prepare
