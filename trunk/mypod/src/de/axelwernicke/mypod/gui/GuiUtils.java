...
public class GuiUtils {
    ...
    /**
     * Base directory where the theme packs for the skin look and feel can be
     * found
     */
    ...
    /**
     * Creates a new instance of GuiUtils. <BR>
     * Since all methods in here are static you should never be in need to call
     * it...
     */
    private GuiUtils() {
    }
    ...
    public static boolean checkVMVersion(java.awt.Component component) {
        ...
        if (majorVersion < 2 && minorVersion < 4) {
            ...
            javax.swing.JOptionPane.showMessageDialog(component, GuiUtils.getStringLocalized("wrongJavaVersionMessage") + jvmVersion, GuiUtils
                    .getStringLocalized("wrongJavaVersionTitle"), javax.swing.JOptionPane.ERROR_MESSAGE);
        }
    ...
    /**
     * Gets the selected clips <BR>- get current clips table view <BR>- get
     * oids for selected clips from the view
     * 
     * @param frontend
     *            to get the clips table view from
     * @return vector, containing the oids of the selected clips
     */
    public static Vector getSelectedClips(Frontend frontend) {
    ...
    /**
     * Gets a string from the resource bundle - localized for the current
     * default language
     * 
     * @param bundle
    ...
    /**
     * Creates and shows the playlist property dialog. <BR>- create property
     * dialog <BR>- set tree model <BR>- set data from playlist to the dialog
     * <BR>- show dialog
     * 
     * @param playlist
     *            to show a property dialog for
     * @param frontend
     *            myPods main window
     * @param backend
    ...
    public static boolean showPlaylistPropertyDialog(Playlist playlist, Frontend frontend, Backend backend) {
        ...
        // set tree
        DefaultTreeModel tree = playlist.isAutoplaylist() ? new DefaultTreeModel(GuiUtils.getAutoPlaylistPropertyTreeModel()) : new DefaultTreeModel(
                GuiUtils.getPlaylistPropertyTreeModel());
    ...
    /**
     * Prepares and shows preferences dialog and evaulates result. <BR>- create
     * preferences dialog <BR>- set tree model <BR>- set data from prefs to
     * the dialog <BR>- show dialog <BR>- copy data from dialog to prefs
     * 
     * @return true, if the dialog was confirmed by the user
     * @param prefs
     *            for myPod
     ...
     * @param clips
     *            clips to change properties for
     * @param frontend
     *            used to calculate the dialogs position
     */
    public static MP3Meta showClipPropertiesDialog(Frontend frontend, Vector clips) {
     ...
     * 
     * @param frontend
     *            where the status line is placed
     */
    public static void updateStatusLine(Frontend frontend) {
        ...
        Vector oids = getSelectedClips(frontend);
        String textToSet = "";
        if (playlist != null) {
            textToSet = (oids != null && !oids.isEmpty())

            ? oids.size() + " of " + playlist.getTotalClips() + " " + GuiUtils.getStringLocalized("clips") + " | "
                    + formatTime(myPod.getBackend().getTotalTime(oids)) + " | " + formatFilesizeMB(myPod.getBackend().getTotalFilesize(oids), 2)
                    + " "

            : playlist.getTotalClips() + " " + GuiUtils.getStringLocalized("clips") + " | " + formatTime(playlist.getTotalTime()) + " | "
                    + formatFilesizeMB(playlist.getTotalFilesize(), 2) + " ";
        }
    ...
    public static Rectangle calcCenteredBounds(Rectangle mainFrame, Rectangle dialog) {
        Rectangle rect;
        rect = new Rectangle((int) (mainFrame.getX() + (mainFrame.getWidth() / 2) - (dialog.getWidth() / 2)), (int) (mainFrame.getY()
                + (mainFrame.getHeight() / 2) - (dialog.getHeight() / 2)), (int) dialog.getWidth(), (int) dialog.getHeight());
        return rect;
    }
     ...
     */
    public static Hashtable getLookAndFeelInfo() {
        Hashtable lookAndFeelInfo = new Hashtable(20);
        ...
        try {
            ...
            for (int i = 0; themes != null && i < themes.length; i++) {
                lookAndFeelInfo.put("Skin Look and Feel (" + themes[i].getName() + ")", themes[i].getName()
    ...
    /**
     * Sets the look and feel from an info string as used in the preferences
     * dialog class.
     * 
     * @param lookAndFeelInfo
     *            describing the look and feel to set
     * @param frame
     *            to set the look and feel for
     */
     ...
     * 
     * @param lfClass
     *            the look and feel class to activate
     * @param frame
     *            root frame that is reinitialized after changing the l&f
     */
     ...
     * 
     * @param anchor
     *            in the help file to show in the browser
     */
