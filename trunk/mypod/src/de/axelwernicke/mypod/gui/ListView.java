...
public class ListView extends javax.swing.JTable {
    ...
    /**
     * Gets the with of values for a column of the table as vector
     * 
     * @param columnIndex
     *            index of the column to get width values for
     ...
     *            empty columns width is set to 0 if this is true
     */
    ...
    public void hideSpreadViewColumn(String viewName, de.axelwernicke.mypod.Preferences preferences) {
        ...
        if (viewName != null && preferences != null) {
            ...
            if (viewName.equals("artistView")) {
                hideColumnIndex = preferences.getListViewColumnInfo()[ClipsTableUtils.ARTIST_INDEX][2];
            } else if (viewName.equals("albumView")) {
                hideColumnIndex = preferences.getListViewColumnInfo()[ClipsTableUtils.ALBUM_INDEX][2];
            } else if (viewName.equals("genreView")) {
                hideColumnIndex = preferences.getListViewColumnInfo()[ClipsTableUtils.GENRE_INDEX][2];
            }
    ...
    /**
     * gets tracks from ListView for DnD - drag from list view. oids are determined from filename via data pool
     * 
     * @return Vector of oid's
     */
    public Vector getSelectedClips() {
        ...
        Vector result = new Vector(selectedCnt);
        ...
        for (int i = 0; i < selectedCnt; i++) {
            try {
                ...
                filename = (String) model.getValueAt(selected[i], ClipsTableUtils.FILEPATH_INDEX);
     ...
     * @return
     */
    public Vector getClips() {
        return getSelectedClips();
    }

    /**
     * sets clips to the list view. Actually this method is called if a clip is dropped to the list view. Please note, that this method is not implemented yet.
     * 
     * @param oids
     *            vector of oids dropped
     */
    public void setClips(Vector oids) {
