...
/**
 * Model for a list view table. The model is mostly a wrapper around a playlist.
 * Additionally all the stuff to sort a table by columns is in here...
 * 
...
public class ClipsTableModel extends javax.swing.table.AbstractTableModel implements javax.swing.event.TableModelListener {
    ...
    /**
     * Checks if the data in the model has the same count of rows as the sorting
     * index array.
     * 
    ...
    /**
     * validates list view table model. check playlist.size() == sortIdx.length
    ...
    /**
     * Sort the model. <br- check the integrity of the model <br- kick
     * (shuttle-)sorting <br- fire table changed event
     */
    public void sort() {
        // check integrity
        validate();
        ...
        // do sorting
        shuttlesort((int[]) sortIdx.clone(), sortIdx, 0, sortIdx.length);

        // log statistics
        double duration = System.currentTimeMillis() - start;
        logger.info("Compares: " + compares + " time: " + duration / 1000.0 + "s, equals to " + compares / (duration / 1000.0)
                + " compares per second.");
