...
/**
 * Remove Clips Action class. removes a clip from the selected playlist. No clips are harmed by this action :) determine selected clip(s) determine selected playlist show dialog to
 * confirm action remove clips from the playlist
 */
public class RemoveClipsPlaylistAction extends AbstractAction {
    ...
    public void actionPerformed(ActionEvent evt) {
        ...
        try {
            ...
            Vector oids = clipsTableView.getSelectedClips();

            // * show dialog to confirm action
            if (oids.size() > 0) {
                int result = JOptionPane.showConfirmDialog(null, GuiUtils.getStringLocalized("doYouWantToRemoveClipsFromPlaylist"), GuiUtils
                        .getStringLocalized("confirmRemoveClips"), JOptionPane.YES_NO_OPTION);
