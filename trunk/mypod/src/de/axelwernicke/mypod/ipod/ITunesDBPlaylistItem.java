 ...
 * It represents a single playlist. Some meta information are stored in two mhod
 * records which this record is followed by. The first playlist item has the
 * name of the ipod attached in the mhods and contains a list of all songs. For
 * others the name of the playlist is stored in the mhod records.
 * 
 * <PRE>
 * 
 * <BR>
 ...
 * 0x0000 'mhyp' - tag <BR>
 * 0x0004 - tag size <BR>
 * 0x0008 - size of the complete playlist in bytes <BR>
 * 0x000C 2 - ?? number of mhod records following ?? <BR>
 * 0x0010 - number of mhip/mhod pairs in the playlist <BR>
 * 0x0014 0/1 - 1 for masterplaylist, 0 else <BR>
 * 0x0018 // TODO - see dechdoc <BR>
 * 0x0022 // TODO - see dechdoc <BR>
...
class ITunesDBPlaylistItem {
    ...
    int addClip(int fileIndex) {
        ...
        ITunesDBContentItem contentItem = new ITunesDBContentItem(ITunesDBContentItem.PLAYLISTENTRY,
        // this.songCount + 1,
        ...
        this.songItems.addElement(indexItem);
        this.songCount++;

        // revalidate size
        // record size of an index item does _not_ contain size of attached
        // content items !
    ...
    long removeClip(long fileIndex) {
        ...
        for (int i = 0; i < songItems.size(); i++) {
            ...
            // since at least the first two enties are mhod, we have to check
            // what we got
            if (indexItem instanceof de.axelwernicke.mypod.ipod.ITunesDBPlaylistIndexItem) {
                ...
                if (playlistIndexItem.getSongIndex() == fileIndex) {
                    // we found the clip, so determine the _complete_ size - the
                    // mhod(s) sizes are not counted in the
                    // in the index items size field...
                    for (int cc = 0; cc < playlistIndexItem.getContentCount(); cc++) {
    ...
    /**
     * Validates clip related data in a playlist. positions of a clip are set in
     * the mhod record total song count is set correctly
     */
    void validateClips() {
        int cnt = 0;
        boolean found;
        ...
        for (int i = 0; i < songItemCount; i++) {
            ...
            // since at least the first two enties are hod, we have to check
            // what we got
            if (indexItem instanceof de.axelwernicke.mypod.ipod.ITunesDBPlaylistIndexItem) {
                ...
                for (int index = 0; index < contentItemCount; index++) {
                    ...
                    if (content.getContentTyp() == content.PLAYLISTENTRY) {
                        found = true;
    ...
    String getName() {
        ...
        try {
            if (this.contentCount > 0) {
                ...
                name = ITunesDBParser.uTF16LittleEndianToString(((ITunesDBContentItem) songItems.elementAt(0)).getContent());
            }
        } catch (Exception e) {
            // TODO logging !
            e.printStackTrace();
    ...
    void setName(String name) {
        if (this.contentCount == 0) // TODO what happens if playlist contains
                                    // already some items ( and a name )
        {
            ...
            this.songItems.addElement(content);
            this.songItems.addElement(content);
    ...
    public String toString() {
        return new StringBuffer("[tagSize] ").append(tagSize).append('\t').append("[recordSize] ").append(recordSize).append('\t').append(
                "[contentCount] ").append(contentCount).append('\t').append("[songCount] ").append(songCount).append('\t').append("[listType] ")
                .append(listType).toString();
    }
     ...
     * 
     * @param songCount
     *            New value of property songCount.
     */
     ...
     * 
     * @param listType
     *            New value of property listType.
     */
     ...
     */
    public java.util.Vector getSongItems() {
        return songItems;
    }
     ...
     * 
     * @param songItems
     *            New value of property songItems.
     */
    public void setSongItems(java.util.Vector songItems) {
