 ...
 * references to the songs content. mhit: 0x0000 'mhit' - tag 0x0004 - tag size
 * 0x0008 - item size 0x000C - number of song content tags for this song 0x0010 -
 * record index ( smth. like 2001 ) 0x0014 1 - unknown6 0x0018 - unknown7 0x001C -
 * unknown8 - special format !! 0x101 -> VBR, 0x100 ->256k seems to be mostly
 * 256, and bitrate itself is stored in unknown 15 0x0020 - date (Mac format,
 * sec since 01.01.1904) 0x0024 - filesize 0x0028 - song duration in ms 0x002C -
 * number of track in album 0x0030 - unknown13 0x0034 - unknown14 0x0038 -
 * bitrate 0x003C - unknown16 // TODO sample rate shifted 16bit left 0x0040 0 -
 * Volume adjustment between -100 and +100 0x0044 0 - Start time in 1000's of
 * sec (where in the song to start playing) 0x0048 0 - Stop time in 1000's of
 * sec (where in the song to stop playing) 0x004C varies - Number of times song
 * has been played 0x0050 varies - same as previous field? 0x0054 varies - Date
 * song was last played
 * 
...
public class ITunesDBSongItem {
    ...
    public String toString() {
        return new StringBuffer("[tagSize] ").append(tagSize).append('\t').append("[recordSize] ").append(recordSize).append('\t').append(
                "[contentCount] ").append(contentCount).append('\t').append("[recordIndex] ").append(recordIndex).append('\t').append("[unknown6] ")
                .append(unknown6).append('\t').append("[unknown7] ").append(unknown7).append('\t').append("[unknown8] ").append(unknown8)
                .append('\t').append("[date] ").append(new java.util.Date(ITunesDBParser.macDateToDate(date)).toString()).append('\t').append(
                        "[date] ").append(date).append('\t').append("[filesize] ").append(filesize).append('\t').append("[duration] ").append(
                        duration).append('\t').append("[trackNumber] ").append(trackNumber).append('\t').append("[unknown13] ").append(unknown13)
                .append('\t').append("[unknown14] ").append(unknown14).append('\t').append("[bitrate] ").append(bitrate).append('\t').append(
