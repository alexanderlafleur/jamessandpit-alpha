 ...
 * the iPod. All information are stored in a binary file, and is contained in
 * tags. The set of tags contains
 * 
 * <PRE>
 * 
...
public class ITunesDB {
    ...
    /**
     * Constructs a new iTunesDB object. An empty, but valid database is
     * created.
     * 
     * @param iPodName ,
    ...
    public void initSonglistHolder() {
        ...
        songlistHolder.setListType(songlistHolder.SONGLIST);
    ...
    public long getTotalFilesize(Vector fileIdc) {
        ...
        if (songlistHeader != null) {
            ...
            for (Iterator iter = fileIdc.iterator(); iter.hasNext();) {
                totalSize += this.getFilesize(((Integer) iter.next()).longValue());
            }
        }
    ...
    public String getFilename(long fileIndex) {
        ...
        for (int i = 0; i < songlistHeader.getSongCount() && !found; i++) {
            ...
            if (songItem.getRecordIndex() == fileIndex) {
                ...
                for (Iterator contentIter = songItem.getContentIterator(); !found && contentIter.hasNext();) {
                    contentItem = (ITunesDBContentItem) contentIter.next();
    ...
    /**
     * removes a song from the song list and all playlists. <BR>- iterate over
     * all playlists and remove clip by fileindex (filename without extension)
    ...
    public void addClip(int fileIndex, de.axelwernicke.mypod.MP3Meta meta) {
        ...
        ITunesDBPlaylistItem masterPlaylist = (ITunesDBPlaylistItem) playlistHeader.getPlaylist(0);
    ...
    /**
     * Validates the database. Record sizes and content count is checked. <BR>-
     * check db record size <BR>- check songlist holder size <BR>- check all
     * song items size <BR>- check song item count <BR>- check playlist holder
     * size <BR>- check all playlist items <BR>- check playlist count <BR>-
     * check song count for each playlist
     * 
    ...
    private boolean tagSizesValid(boolean correct) {
        ...
        if (size != this.songlistHolder.getRecordSize()) {
            valid = false;
            logger.warning("songlist holder size invalid ( record size is: " + this.songlistHolder.getRecordSize() + ", but was calculated as:"
                    + size + ")");
    ...
    public String toString() {
        return new StringBuffer("[tagSize] ").append(tagSize).append('\t').append("[recordSize] ").append(recordSize).append('\t').append(
                "[unknown3] ").append(unknown3).append('\t').append("[unknown4] ").append(unknown4).append('\t').append("[unknown5] ").append(
                unknown5).toString();
    }
    ...
    private boolean referencesValid(boolean correct) {
        ...
        for (int i = 0; i < this.playlistHeader.getPlaylistCount(); i++) {
            ...
            playlistItem = this.playlistHeader.getPlaylist(i);
            for (Iterator indexIter = playlistItem.getSongItems().iterator(); indexIter.hasNext();) {
                ...
                if (indexItem instanceof de.axelwernicke.mypod.ipod.ITunesDBPlaylistIndexItem
                        && !this.songlistHeader.containsClip(((ITunesDBPlaylistIndexItem) indexItem).getSongIndex())) {
                    valid = false;
                    logger.warning("playlist " + playlistItem.getName() + " contains a clip [index]: "
                            + ((ITunesDBPlaylistIndexItem) indexItem).getSongIndex() + " that is not contained in the global song list.");
                }
