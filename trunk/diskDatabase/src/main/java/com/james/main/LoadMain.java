...
public class LoadMain extends BaseMain {
	...
	public static void main(String args[]) throws InvalidFileItemException {
		...
		try {
			...
			System.out.println("There was an error creating disk: " + diskName
					+ ": " + e.getMessage());
			e.printStackTrace();
			String result = main
					.getInput("Do you wish to delete this disk and load it again? Y/N");
	...
	private File askForBaseDir() {
		...
		while (true) {
			...
			try {
				System.out
						.println("Enter base load directory e.g. Press enter for "
								+ defaultBaseName);
			...
			try {
				...
				try {
					...
					try {
						...
						// error
						System.out
								.println("Media number already exists. Choose another.");
						continue;
	...
	private WalletDTO chooseWallet() {
		...
		while (true) {
			...
			try {
				...
				// of disks)
				WalletDTO wallet = getWalletHelper().loadByName(walletName,
						true);

				return wallet;

			} catch (NotFoundException e) {
				boolean createNew = getBooleanInput("Wallet " + walletName
	...
	public void createDisk(WalletDTO wallet, String diskName, long mediaNumber,
			File baseDir) {
