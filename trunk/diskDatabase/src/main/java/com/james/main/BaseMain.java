...
public abstract class BaseMain {
	...
	private boolean parseBooleanQuestion(String value) {
		return ("true".equalsIgnoreCase(value) || "yes".equalsIgnoreCase(value) || "y"
				.equalsIgnoreCase(value));
	}

	public abstract Log getLogger();

	protected Long getLongInput(String msg, BufferedReader in)
			throws IOException {
	...
	private void print(List wallets) {
	...
	private void printFullPath(DirDTO dir) {
		getLogger().info(dir);

		if (dir.getParent() == null) {
			return;
		}
		printFullPath(dir.getParent());
	}

	// private void printFullPath(DiskFileDTO file) {
	// getLogger().info(file);
	// printFullPath(file.getParent());
	// }

	// private void printFullPath(String fileId) {
	// getLogger().info("Logging full path");
	//
	// try {
	// DiskFileDTO file = diskFileHelper.loadDTO(fileId);
	// printFullPath(file);
	//
