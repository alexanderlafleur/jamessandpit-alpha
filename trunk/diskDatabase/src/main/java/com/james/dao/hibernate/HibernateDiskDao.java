...
public class HibernateDiskDao extends HibernateDaoSupport implements DiskDao {
	...
	public Disk findDisk(long rootId) {
		List results = getHibernateTemplate().find(
				"from Disk where root_Id = ?", "" + rootId);

		Disk disk;

		if (results.size() == 0) {
			throw new NotFoundException("Unable to find Disk with root id of: "
					+ rootId);
			...
			throw new NotFoundException(
					"More than one disk found with root id of: " + rootId);
	...
	public Disk findDiskByMediaNumber(long mediaNumber) {
		List results = getHibernateTemplate().find(
				"from Disk where mediaNumber = ?", "" + mediaNumber);

		Disk disk;

		if (results.size() == 0) {
			throw new NotFoundException(
					"Unable to find Disk with mediaNumber of: " + mediaNumber);
			...
			throw new NotFoundException(
					"More than one disk found with mediaNumber of: "
							+ mediaNumber);
	...
	public List getOrphanedDisks() {
		return getHibernateTemplate()
				.find(
						"select disk from Disk disk where disk.id not in (select wallet.disks.id from Wallet wallet)");
	...
	public List getRootOrphanedRoots() {
		return getHibernateTemplate()
				.find(
						"select dir from Dir dir where dir.parent is null and dir.id not in (select disk.root from Disk as disk)");
	...
	public List loadAll(long walletId) {
		List results = getHibernateTemplate().find(
				"from Disk where wallet_Id = ?", "" + walletId);
		return results;
	}

	public Disk loadByName(String label) {
		List results = getHibernateTemplate().find("from Disk where label = ?",
				"" + label);

		Disk disk;

		if (results.size() == 0) {
			throw new NotFoundException("Unable to find Disk with label of: "
					+ label);
			...
			throw new NotFoundException(
					"More than one disk found with label of: " + label);
	...
	public List search(String label) {
		List results = getHibernateTemplate().find(
				"from Disk where label like ?", label);
		return results;
	}
