...
public class URLChat extends JFrame implements ActionListener {
    ...
    private void askWho()
    /*
     * Message format: ChatServlet?cmd=who The message does not require a name argument or a cookie. The response is a list of who is currently logged
     * on, or "no". The list is displayed in the jtsMesgs text area.
     */
    {
        try {
     ...
     */
    {
        ...
        jtfMsg.addActionListener(this); // pressing enter triggers sending of
        // name/score
    ...
    private void sayBye()
    /*
     * ChatServlet?cmd=bye&name=?? + uid cookie The bye message signals the client's departure to the server. The response will be "ok" or "no", but
     * the client exits whatever the response.
     */
    {
        try {
    ...
    private void sayHi()
    /*
     * Message format: ChatServlet?cmd=hi&name=?? The client asks to join the chat group. The server returns a uid cookie as a header field, or
     * rejects the client. A rejection causes the client to exit. The textual response can be "ok" or "no".
     */
    {
        try {
            ...
            if (cookieStr != null) {
                ...
                if (index != -1) {
                    cookieStr = cookieStr.substring(0, index); // strip away
                    // extras
                }
            }
            BufferedReader br = new BufferedReader(new InputStreamReader(conn.getInputStream()));
    ...
    /*
     * synchronized public void showMsg(String msg) // Synchronized since this method can be called by this // object and the URLChatWatcher thread. {
     * jtaMesgs.append(msg); }
     */
    private void sendURLMessage(String msg)
    /*
     * Message format: ChatServlet?cmd=msg&name=??&msg=?? + uid cookie The message is added to the servlet's list. The response is "ok" or "no".
     */
    {
        try {
            URL url = new URL(SERVER + "?cmd=msg&name=" + URLEncoder.encode(userName, "UTF-8") + "&msg=" + URLEncoder.encode(msg, "UTF-8"));
            URLConnection conn = url.openConnection();
            conn.setRequestProperty("Cookie", cookieStr);
            BufferedReader br = new BufferedReader(new InputStreamReader(conn.getInputStream()));
            String response = br.readLine().trim();
    ...
    /*
     * We're updating the messages text area, so the code should be carried out by Swing's event dispatching thread, which is achieved by calling
     * invokeLater(). msg must be final to be used inside the inner class for Runnable. showMsg() may be called by this object and the URLChatWatcher
     * thread, but the updates are serialised by being placed in the queue associated with the event dispatcher. This means that there's no need to
     * synchronize this method. Thanks to Rachel Struthers (rmstruthers@mn.rr.com)
     */
    {
