...
public class PropManager {
    ...
    public PropManager(String loadFnm, boolean hasCoordsInfo) {
        this.filename = loadFnm;
        this.xRot = 0.0;
        this.yRot = 0.0;
        this.zRot = 0.0; // initial loaded object settings
        this.rotInfo = new ArrayList();
        this.scale = 1.0;

        this.t3d = new Transform3D(); // setup reusable Transform3D objects
        this.chgT3d = new Transform3D();
     ...
     * 
     * The loaded object has 4 transform groups above it -- objBoundsTG is for adjusting the object's bounded sphere so it is centered at (0,0,0) and has unit radius.
     * 
     * The other TGs are for doing separate moves, rotations, and scaling of the object. moveTG-->rotTG-->scaleTG-->objBoundsTG-->object
     */
    {
        ...
        this.scaleTG.setCapability(TransformGroup.ALLOW_TRANSFORM_WRITE);
        this.scaleTG.addChild(objBoundsTG);

        // create a transform group for rotating the object
        this.rotTG = new TransformGroup();
        this.rotTG.setCapability(TransformGroup.ALLOW_TRANSFORM_READ);
        this.rotTG.setCapability(TransformGroup.ALLOW_TRANSFORM_WRITE);
        this.rotTG.addChild(this.scaleTG);

        // create a transform group for moving the object
        this.moveTG = new TransformGroup();
        this.moveTG.setCapability(TransformGroup.ALLOW_TRANSFORM_READ);
        this.moveTG.setCapability(TransformGroup.ALLOW_TRANSFORM_WRITE);
    ...
    /*
     * Obtain coords info from the coordinates file for fnm. The coords file has the format: <3D object fnm> [-p px py pz] [-r sequence of numbers] [-s scale]
     */
    {
    ...
    /*
     * These methods are called when applying the coords file information *and* when user commands sent from the GUI are being processed.
     */
    ...
    {
        this.moveTG.getTransform(this.t3d); // get current position from TG
        this.chgT3d.setIdentity(); // reset change Trans
        this.chgT3d.setTranslation(theMove); // setup move
        this.t3d.mul(this.chgT3d); // 'add' move to current position
        this.moveTG.setTransform(this.t3d); // update TG
    } // end of doMove()
    ...
    // rotate the object about the axis by radians amount
    {
        ...
        this.rotTG.getTransform(this.t3d); // get current rotation from TG
        this.chgT3d.setIdentity(); // reset change Trans
        switch (axis) { // setup new rotation
        case X_AXIS:
            this.chgT3d.rotX(radians);
            break;
        case Y_AXIS:
            this.chgT3d.rotY(radians);
            break;
        case Z_AXIS:
            this.chgT3d.rotZ(radians);
            break;
        default:
            System.out.println("Unknown axis of rotation");
            break;
        }
        this.t3d.mul(this.chgT3d); // 'add' new rotation to current one
        this.rotTG.setTransform(this.t3d); // update the TG
    } // end of doRotate()
    ...
    {
        this.xRot = (this.xRot + degrees) % 360; // update x-axis total rotation
        if (degrees == ROT_INCR) {
            this.rotInfo.add(new Integer(1)); // rotation number
    ...
    {
        this.yRot = (this.yRot + degrees) % 360; // update y-axis total rotation
        if (degrees == ROT_INCR) {
            this.rotInfo.add(new Integer(3)); // rotation number
        } else if (degrees == -ROT_INCR) {
    ...
    {
        this.zRot = (this.zRot + degrees) % 360; // update z-axis total rotation
        if (degrees == ROT_INCR) {
            this.rotInfo.add(new Integer(5)); // rotation number
        } else if (degrees == -ROT_INCR) {
    ...
    {
        this.scaleTG.getTransform(this.t3d); // get current scale from TG
        this.chgT3d.setIdentity(); // reset change Trans
        this.chgT3d.setScale(d); // set up new scale
        this.t3d.mul(this.chgT3d); // multiply new scale to current one
        this.scaleTG.setTransform(this.t3d); // update the TG

        this.scale *= d; // update scale variable
    } // end of scale()
    ...
    {
        String coordFnm = "models/" + getName(this.filename) + "Coords.txt";
        try {
            PrintWriter out = new PrintWriter(new FileWriter(coordFnm));

            out.println(this.filename); // object filename
            Vector3d currLoc = getLoc();
            out.println("-p " + this.df.format(currLoc.x) + " " + this.df.format(currLoc.y) + " " + this.df.format(currLoc.z));
            out.print("-r ");
            for (int i = 0; i < this.rotInfo.size(); i++) {
                out.print("" + ((Integer) this.rotInfo.get(i)).intValue());
            }
            out.println("");

            out.println("-s " + this.df.format(this.scale));
    ...
    // {
    // System.out.println(id + " x: " + this.df.format(t.x) + ", " + id + " y: " + this.df.format(t.y) + ", " + id
    // + " z: " + this.df.format(t.z));
    // } // end of printTuple()
