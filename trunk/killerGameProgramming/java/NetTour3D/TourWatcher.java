...
public class TourWatcher extends Thread {
    ...
    /*
     * Create a visitor to the world on this machine. Message format: create <name> xPosn zPosn The visitor has the name <name>, and their sprite will
     * be located at (xPosn ,zPosn). The sprite will be the robot, oriented along the +z-axis. <name> and its sprite are stored in the visitors
     * HashMap
     */
    {
        ...
        if (visitors.containsKey(userName)) {
            ...
            DistTourSprite dtSprite = w3d.addVisitor(userName, xPosn, zPosn, 0);
    ...
    /*
     * Message formats: <name> <command> where <command> can be one of: forward | back | left | right | rotCClock | rotClock | bye Most of these
     * commands are to duplicate the movement of a client's sprite in the copy here. Also check if <name> is in the visitors HashMap
     */
    {
        StringTokenizer st = new StringTokenizer(line);
        String userName = st.nextToken();
    ...
    /*
     * Another client sends details about itself to this client. The reason is to render the other client here on this machine. Message format:
     * detailsFor userName xPosn zPosn rotRadians (xPosn,zPosn) is the position of the sprite, with rotRadians its rotation away from facing front.
     ...
     */
    {
        ...
        if (visitors.containsKey(userName)) {
            ...
            DistTourSprite dtSprite = w3d.addVisitor(userName, xPosn, zPosn, rotRadians);
            visitors.put(userName, dtSprite);
        }
    ...
    /*
     * Msg format: wantDetails cliAddr port The client at (cliAddr, port) wants info about this client. The reason is to render the client as a
     * sprite. Ask WrapNetTour3D (w3d) to send the details in a 'detailsFor' message.
     */
    {
        StringTokenizer st = new StringTokenizer(line);
