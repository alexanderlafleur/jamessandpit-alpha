...
public class Obstacles {
    ...
    public Obstacles() {
        this.obs = new boolean[FLOOR_LEN + 1][FLOOR_LEN + 1]; // larger than
        // FLOOR_LEN*FLOOR_LEN
        this.obsBounds = new BoundingSphere[FLOOR_LEN + 1][FLOOR_LEN + 1];
        for (int z = 0; z <= FLOOR_LEN; z++) {
            for (int x = 0; x <= FLOOR_LEN; x++) {
                this.obs[z][x] = false;
                this.obsBounds[z][x] = null;
            }
        }

        this.obsGroup = new Group();

    } // end of Obstacles()

    public void store(String line)
    /*
     * The line will have the format: (x1,z1) (x2,z2) ...
     */
    {
        int x = 0, z = 0;
        String coordStr;
    ...
    private void markObstacle(int x, int z) {
        ...
        this.obs[z + (FLOOR_LEN / 2)][x + (FLOOR_LEN / 2)] = true;
        this.obsBounds[z + (FLOOR_LEN / 2)][x + (FLOOR_LEN / 2)] = new BoundingSphere(new Point3d(x, 0.0, z), RADIUS);

        this.obsGroup.addChild(makeObs(x, z));
    ...
    /*
     * posn is not allowed to be outside the floor area. Also, every obstacle's bounding sphere is checked against pos
     */
    {
        ...
        for (int z = 0; z <= FLOOR_LEN; z++) {
            for (int x = 0; x <= FLOOR_LEN; x++) {
                if (this.obs[z][x]) {
                    if (this.obsBounds[z][x].intersect(bs)) {
                        return true;
                    }
                }
            }
    ...
    // a red cylinder whose base is on (x,z)
    {
        ...
        Cylinder obs = new Cylinder(RADIUS, HEIGHT, Primitive.GENERATE_NORMALS, obsApp);
