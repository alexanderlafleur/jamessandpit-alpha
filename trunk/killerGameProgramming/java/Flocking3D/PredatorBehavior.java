...
public class PredatorBehavior extends FlockBehavior {
    ...
    /*
     * Initialise the Boidslist of PredatorBoid objects and add the TransformGroups for these boids to boidsBG. Boid is a subclass of TransformGroup
     * so can be added to boidsBG directly.
     */
    {
        PredatorBoid pb;
    ...
    /*
     * Find a nearby PreyBoid (or PreyBoids) and eat it. This causes the PreyBoid to be detached from the scene and from its BoidsList. Return the
     * number of prey eaten (usually 1 or 0). Called by animateBoid() in PredatorBoid.
     */
    {
        ...
        while ((b = (PreyBoid) preyList.getBoid(i)) != null) {
            ...
            if (distFrom.length() < PROXIMITY / 3.0) { // is boid v.close to
                // the prey
    ...
    /*
     * Find all the nearby prey, then move towards their average position by a scaled amount. This is an extra velocity rule used by each
     * PredatorBoid. Called by doVelocityRules() in PredatorBoid.
     */
    {
