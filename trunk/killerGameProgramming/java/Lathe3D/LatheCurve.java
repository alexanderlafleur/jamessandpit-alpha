...
public class LatheCurve {
    ...
    /*
     * The number of points in the new sequence depends on how many curve segments are to be added. Each curve segment between two points adds STEP
     * points to the sequence. A (x,y) point where x is negative starts a straight line, so no new points are added. If s is positive, then STEP
    ...
    /*
     * Hermite blending functions. The first two functions blend the two points, the last two blend the two tangents. For an explanation of how the
     * functions are derived, see any good computer graphics text, e.g. Foley and Van Dam.
     */
    ...
    private void makeCurve(double xsIn[], double ysIn[], Point2d startTangent, Point2d endTangent) {
        ...
        xs = new double[numOutVerts]; // resulting sequence after adding extra
        // pts
        ...
        for (int i = 0; i < numInVerts - 1; i++) {
            ...
            // (x,y)
            if (xsIn[i] < 0) {
                xs[startPosn] = Math.abs(xsIn[i + 1]); // in case the next pt
                // is -ve also
    ...
    /*
     * Calculate the Hermite curve's (x,y) coordinates between points (x0,y0) and (x1,y1). t0 and t1 are the tangents for those points. The (x0,y0)
     * point is not included, since it was added at the end of the previous call to makeHermite(). Store the coordinates in the xs[] an ys[] arrays,
     * starting at index startPosn.
     */
    {
