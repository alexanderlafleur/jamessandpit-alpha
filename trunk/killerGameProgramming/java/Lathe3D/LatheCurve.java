...
public class LatheCurve {
     ...
     */
    {
        ...
        if (this.height != ysIn[ysIn.length - 1]) {
    ...
    private void makeCurve(double xsIn[], double ysIn[], Point2d startTangent, Point2d endTangent) {
        ...
        this.xs = new double[numOutVerts]; // resulting sequence after adding extra
        // pts
        this.ys = new double[numOutVerts];

        this.xs[0] = Math.abs(xsIn[0]); // start of curve is initialised
        this.ys[0] = ysIn[0];
        int startPosn = 1;
        ...
        for (int i = 0; i < numInVerts - 1; i++) {
            ...
            // if xsIn[i] < 0 then use a straight line to link (x,y) to next
            // (x,y)
            if (xsIn[i] < 0) {
                this.xs[startPosn] = Math.abs(xsIn[i + 1]); // in case the next pt
                // is -ve also
                this.ys[startPosn] = ysIn[i + 1];
                startPosn++;
            } else { // make a Hermite curve between the two points by adding
                // STEP new pts
                makeHermite(this.xs, this.ys, startPosn, xsIn[i], ysIn[i], xsIn[i + 1], ysIn[i + 1], t0, t1);
                startPosn += (STEP + 1);
            }
        }
    } // end of makeCurve()
    ...
    /*
     * The number of points in the new sequence depends on how many curve segments are to be added. Each curve segment between two points adds STEP points to the sequence.
     * 
     * A (x,y) point where x is negative starts a straight line, so no new points are added. If s is positive, then STEP points will be added.
     */
    {
     ...
     */
    {
        double xLen = Math.abs(xsIn[i + 1]) - Math.abs(xsIn[i - 1]); // ignore
        // any
        // -ve
    ...
    /*
     * Calculate the Hermite curve's (x,y) coordinates between points (x0,y0) and (x1,y1). t0 and t1 are the tangents for those points.
     * 
     * The (x0,y0) point is not included, since it was added at the end of the previous call to makeHermite().
     * 
     * Store the coordinates in the xs[] an ys[] arrays, starting at index startPosn.
     */
    {
    ...
    /*
     * Hermite blending functions. The first two functions blend the two points, the last two blend the two tangents. For an explanation of how the functions are derived, see any
