...
public class NotesSynth {
    ...
    /*
     * Middle C (C4) has a frequency of 262 Hz, doubling with each octave increase (C5, C6, ...), halving with each octave decrease (C3, C2 ...) See
     * http://www.phys.unsw.edu.au/~jw/notes.html for a table.
     */
    ...
    {
        /*
         * Create a stereo, signed PCM output line, using 16 bits per sample in little endian format. Four bytes will be used for each sample/frame.
         */
        format = new AudioFormat(AudioFormat.Encoding.PCM_SIGNED, SAMPLE_RATE, 16, 2, 4, SAMPLE_RATE, false);
        /*
         * SAMPLE_RATE // samples/sec 16 // sample size in bits, values can be -2^15 -- 2^15-1 2 // no. of channels, stereo here 4 // frame size in
         * bytes (2 bytes/sample * 2 channels) SAMPLE_RATE // same as frames/sec false // little endian
         */
    ...
    // play notes through the SourceDataLine
    {
        ...
        int maxSize = Math.round(SAMPLE_RATE * format.getFrameSize() / MIN_FREQ);
        // the frame size is 4 bytes
        byte[] samples = new byte[maxSize];
        line.start();
        /*
         * Generate an increasing pitch sequence, repeated 9 times, each time increasing a bit faster, and the volume decreasing
         */
        double volume;
    ...
    /*
     * We generate a single sine wave of the required amplitude and frequency, equivalent to a single note. The pitch of the sound (how high the note
     * is) depends on the frequency of the wave. The higher the frequency, the higher the pitch. Each note is 16 bit, little endian, stereo --
     * matching the format
     */
    {
