...
public class VolChanger extends Thread {
     ...
     */
    {
        ...
        this.numChanges = duration / PERIOD;
     ...
     */
    {
         ...
         */
        int volume = this.player.getMaxVolume();
        int stepVolume = volume / this.numChanges;
        if (stepVolume == 0) {
            stepVolume = 1;
        }
        ...
        while (counter < this.numChanges) {
            try {
                volume -= stepVolume; // reduce the required volume level
                if ((volume >= 0) && (this.player != null)) {
                    this.player.setVolume(volume); // change the volume
                }
