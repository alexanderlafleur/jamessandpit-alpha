...
public class JumperSprite extends Sprite {
    ...
    /*
     * Falling will continue until the sprite hits the top of a brick. The game only allows a brick ribbon which has a complete floor, so the sprite
     * must eventually touch down. Falling mode can be entered without a corresponding rising sequence, for instance, when the sprite walks off a
     * cliff.
     */
    {
        int yTrans = brickMan.checkBrickTop(xWorld + getWidth() / 2, yWorld + getHeight() + vertStep, vertStep);
    ...
    /*
     * Rising will continue until the maximum number of vertical steps is reached, or the sprite hits the base of a brick. The sprite then switches to
     * falling mode.
     */
    {
    ...
    /*
     * Although the sprite is not moving in the x-direction, we must still update its (xWorld, yWorld) coordinate. Also, if the sprite is jumping then
     * its y position must be updated with moveVertically(). updateSprite() should only be called after collsion checking with willHitBrick()
     */
    {
        if (!isStill) { // moving
            if (isFacingRight) {
    ...
    /*
     * Test if the next x position is inside a brick Ignore any y motion. This method should always be called before updateSprite() makes the actual
     * move.
     */
    {
        if (isStill) {
