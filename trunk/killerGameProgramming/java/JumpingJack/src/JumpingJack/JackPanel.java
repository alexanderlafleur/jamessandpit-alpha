...
public class JackPanel extends JPanel implements Runnable, ImagesPlayerWatcher {
    ...
    public JackPanel(JumpingJack jj, long period) {
        ...
        jack = new JumperSprite(PWIDTH, PHEIGHT, brickMoveSize, bricksMan, imsLoader, (int) (period / 1000000L)); // in ms
    ...
    // called by fireball sprite when it hits jack at (x,y)
    {
        if (!showExplosion) { // only allow a single explosion at a time
            showExplosion = true;
            xExpl = x - explWidth / 2; // \ (x,y) is the center of the
            // explosion
            yExpl = y - explHeight / 2;
            /*
             * Play an explosion clip, but cycle through them. This adds variety, and gets round not being able to play multiple instances of a clip
             * at the same time.
             */
