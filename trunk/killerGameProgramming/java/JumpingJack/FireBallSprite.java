...
public class FireBallSprite extends Sprite {
    ...
    /*
     * If the ball has hit jack, tell JackPanel (which will display an explosion and play a clip), and begin again.
     */
    {
        Rectangle jackBox = this.jack.getMyRectangle();
        jackBox.grow(-jackBox.width / 3, 0); // make jack's bounded box
        // thinner

        if (jackBox.intersects(getMyRectangle())) { // jack collision?
            this.jp.showExplosion(this.locx, this.locy + getHeight() / 2);
