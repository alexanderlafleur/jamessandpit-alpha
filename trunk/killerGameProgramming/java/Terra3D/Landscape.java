...
public class Landscape {
    ...
    /*
     * The land texture (in models/fname.jpg) shows the landscape from above, and is used to colour the mesh loaded from the OBJ file. To save memory,
     * the shape is one-sided. The texture is blended with a white material so that lighting effects can be shown.
     */
    {
    ...
    /*
     * The floor of the landscape is the XY plane, starting at (0,0), with sides of landLength units. The landscape (landBG) is rotated to lie on the
     * XZ plane, scaled to have floor sides of length LAND_LEN (scaleLen = LAND_LEN/landLength), then translated so that the center of the landscape
     * is at (0,0).
     */
    {
    ...
    /*
     * Add 4 textured walls around the landscape. Each wall is a TexturedPlane object, which are placed using world coordinates, and so linked to
     * sceneBG, not landBG. The texture is hardwired to be WALL_PIC.
     */
    ...
    /*
     * If we reach this point, then landShape3D contains a single GeometryArray of points representing the landscape's mesh. We can get the dimensions
     * of the mesh by using a BoundingBox. Check if the XY plane is the 'floor' of the landscape, which is how the TerraGen mesh should have been
     * exported. The X and Y lengths should be the same, and start at (0,0). Set the following vars: landLength // length of the X (and Y) side
     * scaleLen // the scaling necessary to fit landLength // into LAND_LEN units in the world minHeight and maxHeight // along the Z axis
     */
    {
    ...
    /*
     * Check that landBG (the mesh's BranchGroup) contains a single Shape3D, and assign it to landShape3D for later. Also check that the shape holds a
     * single GeometryArray. landShape3D is later used for picking, and its dimensions are extracted.
     */
    {
    ...
    // is world (x,z) in the landscape? Used by KeyBehavior
    {
        ...
        if (landVec.x <= 0 || landVec.x >= landLength || landVec.y <= 0 || landVec.y >= landLength) {
            return false;
        }
        return true;
    } // end of inLandscape()
    ...
    /*
     * The scenery file (models/fname.txt) has the format: start x y z <object file> x y z scale <object file> x y z scale : The start line must be
     * included -- it says where the user is placed in the landscape. The scenery objects are optional. Each scenery object is loaded with a
     * PropManager object; we assume the existence of "coords" data files for the models. The (x,y,z) values are in landscape coordinates, and scale
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (modelFnm.equals("start")) { // user's starting point
                    originVec = landToWorld(xCoord, yCoord, zCoord); // in
                    // world
                    // coords
                    startSet = true;
                } else {
    ...
    /*
     * Adjust the genration planes so that the texture is 'stetched' over the entire mesh. The mesh has equal X- and Y- axis sides of landLength, and
     * the mesh's lower left-hand corner starts at (0,0), so the stretching is quite simple.
     */
    { /*
    ...
    /*
     * Used by KeyBehavior Converting world to land coordinates involves translation, scaling, and rotation. The rotation is achieved by switching the
     * y- and z- values.
     */
    {
        double xCoord = (worldVec.x + LAND_LEN / 2) / scaleLen;
        double yCoord = (-worldVec.z + LAND_LEN / 2) / scaleLen; // z-axis
        // -->
        // y-axis
