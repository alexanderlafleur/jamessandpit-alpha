...
public class MazeGen {
    ...
    /*
     * From the current cell, randomly try to visit all the other cells two steps away. Use depth-first traversal. A cell is not visited more than
     * once, and so not every direction will be opened up with a path.
     */
    {
    ...
    /*
     * Select a starting cell at an even position. Each move after this is in steps of two which ensures that a path is created rather than a big
     * block of space.
     */
    ...
    // convert roughly CYLINDER_PERCENT blocks to cylinders
    {
        for (int x = 0; x < height; x++) {
            for (int y = 0; y < width; y++) {
                if (maze[x][y] == 'b' && Math.random() <= CYLINDER_PERCENT) {
                    maze[x][y] = 'c';
                }
            }
        }
     ...
     */
    {
        for (int y = width / 2; y < width - 3; y++) { // height is odd
    ...
    private void processArgs(String[] args) {
        ...
        System.out.println("Width: " + width + "; Height: " + height + " (2 added for borders)");
