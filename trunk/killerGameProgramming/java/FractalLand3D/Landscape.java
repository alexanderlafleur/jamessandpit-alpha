...
public class Landscape {
    ...
    /*
     * The number of textures are used to calculate the height boundaries between MIN_HEIGHT and MAX_HEIGHT. The first named texture in textureFns[] is used for quads in the first
     * height range (the lowest), the second texture for the next height range, and so on.
     */
    ...
    private Vector3d originVec = new Vector3d(); // stating point for
    // viewpoint
    ...
    public Landscape(double flatness) {
        this.landBG = new BranchGroup();
        this.floorBG = new BranchGroup();
        this.landBG.addChild(this.floorBG); // so landBG-->floorBG

        setTexBoundaries();

        this.picker = new PickTool(this.floorBG); // only check the floor
        ...
        this.vertices = fm.getVertices(); // get the vertices generated by
        // FractalMesh
    ...
    {
        this.textureBoundaries = new double[NUM_TEXTURES];
        double boundStep = (MAX_HEIGHT - MIN_HEIGHT) / NUM_TEXTURES;
        double boundary = MIN_HEIGHT + boundStep;
        for (int j = 0; j < NUM_TEXTURES; j++) {
            this.textureBoundaries[j] = boundary; // place in increasing order
            boundary += boundStep;
        }
    ...
    /*
     * Examine the quads stored in vertices[]. Check the average height and assign it to the ArrayList for coords in that height range.
     * 
     * Pass each ArrayList and their texture filenames to a TexturedPlanes object to build the 3D mesh for those coords.
     */
    {
        ...
        int heightIdx;
        for (int j = 0; j < this.vertices.length; j = j + 4) { // test each quad
            heightIdx = findHeightIdx(j); // which height index applies to the
            // quad
            addCoords(coordsList[heightIdx], j); // add quad to the list for
            // that height
        ...
        for (int i = 0; i < NUM_TEXTURES; i++) {
            if (coordsList[i].size() > 0) {
                this.floorBG.addChild(new TexturedPlanes(coordsList[i], // then add
                        // to the
                        // floor
                        "images/" + textureFns[i]));
            }
    ...
    /*
     * Find the height index for the quad starting at vertices[vertIndex]. Get the average height for the 4 points in the quad. If it is less than the boundary value, then the quad
     * belongs to that height range.
     */
    {
        double ah = avgHeight(vertIndex);
        for (int i = 0; i < this.textureBoundaries.length; i++) {
            if (ah < this.textureBoundaries[i]) {
                return i;
            }
        }
    ...
    {
        return (this.vertices[vi].y + this.vertices[vi + 1].y + this.vertices[vi + 2].y + this.vertices[vi + 3].y) / 4.0;
    }

    private void addCoords(ArrayList coords, int vi)
    // add the 4 coords (the quad) beginning at vertices[vi] to the ArrayList
    {
        coords.add(this.vertices[vi]);
        coords.add(this.vertices[vi + 1]);
        coords.add(this.vertices[vi + 2]);
        coords.add(this.vertices[vi + 3]);
    } // end of addCoords()
    ...
    {
        if (!this.foundOrigin) {
            if ((this.vertices[vi].x == 0.0) && (this.vertices[vi].z == 0.0)) {
                // System.out.println("Found Origin: (" + vertices[vi].x + ", "
                // +
                // vertices[vi].y + ", " + vertices[vi].z + ")");
                this.originVec.y = this.vertices[vi].y;
    ...
    // Add 4 walls around the landscape
    {
        ...
        this.landBG.addChild(new ColouredPlane(p3, p1, p2, p4, new Vector3f(-1, 0, 0), eveningBlue));
        // front wall; counter-clockwise from back
        this.landBG.addChild(new ColouredPlane(p5, p3, p4, p6, new Vector3f(0, 0, -1), eveningBlue));
        // right wall
        this.landBG.addChild(new ColouredPlane(p7, p5, p6, p8, new Vector3f(-1, 0, 0), eveningBlue));
        // back wall
        this.landBG.addChild(new ColouredPlane(p7, p8, p2, p1, new Vector3f(0, 0, 1), eveningBlue));
    } // end of addWalls()
    ...
    /*
     * Throw a pick ray downwards below the (x,z) point to intersect with the floor. Extract the y-value (the height of the floor and return it.
     * 
     * Picking is a bit flakey, especially near quad edges and corners, so if no PickResult is found, no intersections found, or the extraction of intersection coords fails, then
     * the height for the last viewpoint position is returned.
     */
    {
        Point3d pickStart = new Point3d(x, MAX_HEIGHT * 2, z); // start from
        // high up
        this.picker.setShapeRay(pickStart, DOWN_VEC); // shoot a ray downwards
