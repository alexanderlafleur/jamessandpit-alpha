...
public class Landscape {
    ...
    /*
     * The number of textures are used to calculate the height boundaries between MIN_HEIGHT and MAX_HEIGHT. The first named texture in textureFns[]
     * is used for quads in the first height range (the lowest), the second texture for the next height range, and so on.
     */
    ...
    public Landscape(double flatness) {
        ...
        vertices = fm.getVertices(); // get the vertices generated by
        // FractalMesh
        platifyFloor();
        addWalls();
    } // end of Landscape()
    ...
    // Add 4 walls around the landscape
    {
        ...
        landBG.addChild(new ColouredPlane(p3, p1, p2, p4, new Vector3f(-1, 0, 0), eveningBlue));
        // front wall; counter-clockwise from back
        landBG.addChild(new ColouredPlane(p5, p3, p4, p6, new Vector3f(0, 0, -1), eveningBlue));
        // right wall
        landBG.addChild(new ColouredPlane(p7, p5, p6, p8, new Vector3f(-1, 0, 0), eveningBlue));
        // back wall
        landBG.addChild(new ColouredPlane(p7, p8, p2, p1, new Vector3f(0, 0, 1), eveningBlue));
    } // end of addWalls()
    ...
    /*
     * Find the height index for the quad starting at vertices[vertIndex]. Get the average height for the 4 points in the quad. If it is less than the
     * boundary value, then the quad belongs to that height range.
     */
    {
     ...
     * Picking is a bit flakey, especially near quad edges and corners, so if no PickResult is found, no intersections found, or the extraction of
     * intersection coords fails, then the height for the last viewpoint position is returned.
     */
    {
    ...
    /*
     * Examine the quads stored in vertices[]. Check the average height and assign it to the ArrayList for coords in that height range. Pass each
     * ArrayList and their texture filenames to a TexturedPlanes object to build the 3D mesh for those coords.
     */
    {
        ...
        for (int i = 0; i < NUM_TEXTURES; i++) {
            if (coordsList[i].size() > 0) {
                floorBG.addChild(new TexturedPlanes(coordsList[i], // then add
                        // to the
                        // floor
                        "images/" + textureFns[i]));
            }
