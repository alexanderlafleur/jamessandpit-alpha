package FractalLand3D.PlaneEquation;
...
public class PointHeight {
    ...
    /*
     * The pt can be above or below the two triangulation lines defined by SQ and PR. posSQ and posPR can take the values ABOVE or BELOW.
     */
    ...
    public PointHeight(String fnm) {
        ...
        if (inRange(this.pt)) {
            relativeToLines(this.pt);
            calcHeightSQ(this.posSQ, this.pt); // pt relationship with SQ
            calcHeightPR(this.posPR, this.pt); // pt relationship with PR
        } else {
    ...
    /*
     * Obtain coords and point info from fnm, with the format: num num num // P coord num num num // Q coord num num num // R coord num num num // S coord num num // (x,z) of point
     */
    {
        try {
            BufferedReader br = new BufferedReader(new FileReader(fnm));
            this.coordP = getCoord('P', br.readLine());
            this.coordQ = getCoord('Q', br.readLine());
            this.coordR = getCoord('R', br.readLine());
            this.coordS = getCoord('S', br.readLine());

            this.pt = getPoint(br.readLine());
    ...
    // return (x,y,z)
    {
        ...
        System.out.println(ptCh + " coord: (" + this.df.format(pt.x) + ", " + this.df.format(pt.y) + ", " + this.df.format(pt.z) + ")");
        return pt;
    } // end of getCoord()
    ...
    // return (x,0,z); 0 is not used
    {
        ...
        System.out.println("Point: (" + this.df.format(pt.x) + ", " + this.df.format(pt.z) + ")");
        return pt;
    } // end of getCoord()
    ...
    {
        if ((pt.x < this.coordS.x) || (pt.x > this.coordQ.x) || (pt.z < this.coordS.z) || (pt.z > this.coordQ.z)) {
            return false;
        }
        return true;
    } // end of inRange()
    ...
    /*
     * Work out if the pt is above or below the two triangulation lines DB and AC.
     */
    {
        // test of SQ: z = x + (S.z-S.x)
        if (pt.z > (pt.x + this.coordS.z - this.coordS.x)) {
            this.posSQ = ABOVE;
        } else {
            this.posSQ = BELOW;
        }
        ...
        if (pt.z > (-pt.x + this.coordP.z + this.coordP.x)) {
            this.posPR = ABOVE;
        } else {
            this.posPR = BELOW;
        }

        String relSQ = (this.posSQ == ABOVE) ? "Above " : "Below ";
        String relPR = (this.posPR == ABOVE) ? "Above " : "Below ";
        System.out.println("Line Relationships: " + relSQ + "SQ;  " + relPR + "PR");
    } // end of relativeToLines()

    private void calcHeightSQ(int linePos, Vector3d pt)
    /*
     * Calculate the two vectors u, v that define the triangle which uses the SQ triangulation line
     * 
     ...
     */
    {
        ...
        if (linePos == BELOW) {
            System.out.println("\nSQ Triangle SQR");
            u.set(1, this.coordQ.y - this.coordS.y, 1); // Q-S
            v.set(1, this.coordR.y - this.coordS.y, 0); // R-S
            vecsToHeight(u, v, this.coordS);
        } else if (linePos == ABOVE) {
            System.out.println("\nSQ Triangle SPQ");
            u.set(1, this.coordQ.y - this.coordP.y, 0); // Q-P
            v.set(0, this.coordS.y - this.coordP.y, -1); // S-P
            vecsToHeight(u, v, this.coordP);
        } else {
    ...
    /*
     * Calculate the two vectors u, v that define the triangle which uses the PR triangulation line
     * 
     ...
     */
    {
        ...
        if (linePos == BELOW) {
            System.out.println("\nPR Triangle PRS");
            u.set(1, this.coordR.y - this.coordP.y, -1); // R-P
            v.set(0, this.coordS.y - this.coordP.y, -1); // S-P
            vecsToHeight(u, v, this.coordP);
        } else if (linePos == ABOVE) {
            System.out.println("\nPR Triangle RPQ");
            u.set(1, this.coordQ.y - this.coordP.y, 0); // Q-P
            v.set(1, this.coordR.y - this.coordP.y, -1); // R-P
            vecsToHeight(u, v, this.coordP);
        } else {
     ...
     * 
     * Calculate the distance to the origin by using the dot product on the normal and any point covered by the triangle.
     * 
     * The normal is (A, B, C) and distance is D. These define the equation for the triangle: (A * x) + (B * y) + (C * z) = D
     * 
     ...
     */
    {
        ...
        System.out.println("A: " + this.df.format(normal.x) + ", B: " + this.df.format(normal.y) + ", C: " + this.df.format(normal.z) + ", D: " + this.df.format(dist)); // Ax +
                                                                                                                                                                            // By +
                                                                                                                                                                            // Cz =
                                                                                                                                                                            // D

        double height = (dist - (normal.x * pt.x) - (normal.z * pt.z)) / normal.y;
        System.out.println("Height for pt: " + this.df.format(height));
    } // end of vecsToHeight()
