...
// Holds the 3D canvas where the loaded image is displayed
{
    ...
    /*
     * Add a texture to a GeometryArray. Add the texture to all faces of the model, 'stretch' the texture over the model, modulate the texture with
     * the existing colour and lighting components. The texture comes from TEXTURE_FN, and was loaded before the traversal of the model began.
     */
    {
    ...
    /*
     * Shape adjusting examples: change the colour of a shape to blue draw the shape in outline (i.e. as a wireframe) make the shape transparent add a
     * texture to the shape (this can be combined with the colour changing method) The choice of which one depends on adaptNo: 0-3, and 4 does
     * makeBlue() and addTexture()
     */
    {
        switch (adaptNo) {
        case 0:
    ...
    // Scale the model based on its original bounding box size
    {
        ...
        System.out.println("max dimension: " + df.format(max) + "; scaleFactor: " + df.format(scaleFactor));
    ...
    // initilise the scene
    {
        ...
        if (loadedScene != null) {
            ...
            storeGraphInfo(loadedBG); // traverse model's graph; save info to
            // file
            // adjust the model's component shapes in various ways
    ...
    /*
     * A Node can be a Group or a Leaf. Depending on the type of Group or Leaf, report different things. Recursively call examineNode() on the
     * children of a Group, incrementing level so the output can be indented correctly.
     */
    {
    ...
    /*
     * A Shape3D is a container for Appearance and Geometry components. Show appearance info with printAppearance(). A shape may contain many
     * geometries: examine each one with examineGeometry()
     */
    {
    ...
    /*
     * Load the model from fn into the scene graph using a NCSA Portfolio loader. Rotate and scale it to make it easier to see. Store the loaded
     * model's scene in the global loadedScene, and its branch group in loadedBG.
     */
    {
        ...
        try {
            ...
            if (loadedScene != null) {
                ...
                // fix that
                Vector3d scaleVec = calcScaleFactor(loadedBG, fn); // scale the
                // model
    ...
    /*
     * Specify how the texture is mapped to the model by adjusting the genration planes so that a single texture is 'stetched' over the entire model
     * rather than repeatedly 'tiled'. This makes a big difference for models that are very large since the default tiling approach tends to lose
     * texture detail and the display 'shimmers' since the texture is mapped to only a few pixels on the model.
     */
    {
