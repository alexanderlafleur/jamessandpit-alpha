...
public class TreeLimb {
    ...
    public TreeLimb(int axis, double angle, float rad, float len, TransformGroup startLimbTG, TreeLimb par) {
        ...
        if (parent != null) {
            parent.addChildLimb(this); // tell the parent that there's a new
            // child
        }
    } // end of TreeLimb()
    ...
    /*
     * Any methods using the cylinder's current radius or length must first apply the current scaling values (stored in scaleLimb) to the cylinder's
     * original radius and limbLen values. No changes are every made to radius or limbLen, only to the scaling stored in scaleTG.
     */
    ...
    private TransformGroup locateEndLimb() {
        ...
        endLimbTG.setCapability(Group.ALLOW_CHILDREN_EXTEND);
    ...
    // a green cylinder whose base is at (0,0,0)
    {
        ...
        trans1.setTranslation(new Vector3d(0, limbLen / 2, 0)); // move up
        // length/2
        baseTG.setTransform(trans1);
        Appearance app = new Appearance();
        limbMaterial = new Material(black, black, green, brown, 50.f); // specular,
        // 100
    ...
    /*
     * Since endLimbTG is not attached to Cylinder, it will not automatically be adjusted when the cylinder is scaled (e.g. lengthened, shortened).
     * This means that we must calculate the new position for endLimbTG ourselves. yScale is the amount that the Cylinder is about to be scaled, which
     ...
     */
    {
        ...
        double changedLen = limbLen * (1.0 - OVERLAP) * yScale - currLimbLen;
    ...
    {
        double scaledLimbLen = limbLen * scaleLimb.y;
    ...
    {
        double scaledRadius = radius * scaleLimb.x;
