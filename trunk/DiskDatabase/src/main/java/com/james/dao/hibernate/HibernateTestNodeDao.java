...
public class HibernateTestNodeDao extends HibernateDaoSupport implements
		TestNodeDao {
	...
	public TestNode load(long nodeId) {
		return (TestNode) getHibernateTemplate().load(TestNode.class,
				new Long(nodeId));
	}
	...
	public List search(String name) {
		List results = getHibernateTemplate().find(
				"from TestNode where name like ?", name);
		return results;
	}
