...
public class SearchMain extends BaseMain {
    ...
    public static void main(String args[]) throws IOException {
        ...
        while (true) {
            ...
            Long low = main.getLongInput("Enter lower size to search for: ", in);
            Long high = main.getLongInput("Enter high size to search for: ", in);
            String album = main.getInput("Enter album to search for: ", in);
            String artist = main.getInput("Enter artist to search for: ", in);

            DiskFileSearchCriteria criteria = new DiskFileSearchCriteria(name, type, path, low, high);

            List files = main.searchForFiles(criteria);

            main.printFiles(files);

            MP3SearchCriteria mp3Criteria = new MP3SearchCriteria(name, type, low, high, album, artist);
