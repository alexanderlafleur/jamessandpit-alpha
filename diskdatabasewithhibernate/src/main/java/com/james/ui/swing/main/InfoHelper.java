...
public class InfoHelper {
    ...
    public BaseInfo convert(DTO dto, BaseNodeHelper nodeHelper) {
        ...
        if (dto instanceof DiskFileDTO) {
            ...
            node = new FileInfo(fileDTO.getId(), fileDTO.getName(), fileDTO, nodeHelper);

        } else if (dto instanceof MP3DTO) {
            MP3DTO mp3DTO = (MP3DTO) dto;
            logger.debug("Converting mp3: " + mp3DTO.getName());

            node = new MP3Info(mp3DTO.getId(), mp3DTO.getName(), mp3DTO, nodeHelper);

        } else {
            throw new ConversionException("Unknown dto type: " + dto.getClass().getName());
        }
    ...
    public BaseInfoWrapper wrap(DTO dto, BaseNodeHelper nodeHelper) {
        ...
        if (dto instanceof DiskFileDTO) {
            ...
            node = new FileInfoWrapper(fileDTO.getId(), fileDTO.getName(), fileDTO, nodeHelper);

        } else if (dto instanceof MP3DTO) {
            MP3DTO mp3DTO = (MP3DTO) dto;

            node = new MP3InfoWrapper(mp3DTO.getId(), mp3DTO.getName(), mp3DTO, nodeHelper);
