...
public class HibernateWalletDao extends HibernateDaoSupport implements WalletDao {
    ...
    public Wallet findWallet(long diskId) {
        ...
        List results = getHibernateTemplate().find("select wallet from Wallet wallet where wallet.disks.id in (select disk.id from Disk disk where disk.id = ?)", "" + diskId);
        ...
        if (results.size() == 0) {
            throw new NotFoundException("Unable to find Disk with diskId of: " + diskId);

        } else if (results.size() == 1) {
            wallet = (Wallet) results.get(0);

        } else {
            throw new NotFoundException("More than one disk found with id of: " + diskId);
    ...
    public List<Wallet> load() {
        List<Wallet> wallets = getHibernateTemplate().loadAll(Wallet.class);

        return wallets;
    }

    public Wallet load(long walletId) {
        return (Wallet) getHibernateTemplate().load(Wallet.class, new Long(walletId));
    }

    public Wallet loadByDescription(String description) {
        List results = getHibernateTemplate().find("from Wallet where description = ?", description);

        Wallet wallet;

        if (results.size() == 0) {
            throw new NotFoundException("Unable to find wallet with description of: " + description);

        } else if (results.size() == 1) {
            wallet = (Wallet) results.get(0);

        } else {
            throw new NotFoundException("More than one wallet found with description of: " + description);
    ...
    public List<Wallet> search(String description) {
        List<Wallet> results = getHibernateTemplate().find("from Wallet where description like ?", description);
        return results;
    }
