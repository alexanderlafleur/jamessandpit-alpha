...
public class Loader3D extends JFrame implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        } else if (e.getSource() == xRotLeftBut) {
    ...
    /*
     * The GUI consists of the 3D canvas in the center, and a column of buttons, textfields, etc., down the right hand side (the control panel). The
     * top half of the control panel are for inputs: a series of buttons for carrying out moves and rotations, and a textfield for entering scaling
     * info. The bottom half of the panel is for displaying the current position, rotation, and scaling details.
     */
    {
    ...
    private void showPosInfo() {
        ...
        xyzTF.setText("( " + df.format(loc.x) + ", " + df.format(loc.y) + ", " + df.format(loc.z) + " )");
    }

    private void showRotInfo() {
        Point3d rots = w3d.getRotations();
        rotTF.setText("( " + df.format(rots.x) + ", " + df.format(rots.y) + ", " + df.format(rots.z) + " )");
    }
