...
public class Worm {
    ...
    public void move()
    /*
     * A move causes the addition of a new dot to the front of the worm, which becomes its new head. A dot has a position and compass
     * direction/bearing, which is derived from the position and bearing of the old head. move() is complicated by having to deal with 3 cases: when
     * the worm is first created when the worm is growing when the worm is MAXPOINTS long (then the addition of a new head must be balanced by the
     * removal of a tail dot)
     */
    {
    ...
    {
        if (nPoints > 0) {
            if (Math.abs(cells[headPosn].x + RADIUS - x) <= DOTSIZE && Math.abs(cells[headPosn].y + RADIUS - y) <= DOTSIZE) {
                return true;
            }
        }
        return false;
    ...
    /*
     * Create new head position and compass direction/bearing. This has two main parts. Initially we try to generate a head by varying the old
     * position/bearing. But if the new head hits an obstacle, then we shift to a second phase. In the second phase we try a head which is 90 degrees
     * clockwise, 90 degress clockwise, or 180 degrees reversed so that the obstacle is avoided. These bearings are stored in fixedOffs[].
     */
    {
        Point newPt;
        int newBearing;
    ...
    /*
     * Return the next coordinate based on the previous position and a compass bearing. Convert the compass bearing into predetermined increments
     * (stored in incrs[]). Add the increments multiplied by the DOTSIZE to the old head position. Deal with wraparound.
     */
    {
