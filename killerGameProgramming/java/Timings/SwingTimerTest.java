...
public class SwingTimerTest extends JPanel implements ActionListener {
     ...
     * 
     */
    private static final long serialVersionUID = -1138604486081934804L;
     ...
     * take), the actual elapsed time in this interval, the error between these two numbers; - the total number of calls to paintComponent(); - the
     * FPS (frames/sec) for this interval, and the average FPS over the last NUM_FPSs intervals. The data is collected every MAX_STATS_INTERVAL (1
     * sec).
     */
    {
        frameCount++;
        ...
        if (statsInterval >= MAX_STATS_INTERVAL) { // record stats every
            ...
            System.out.println(timedf.format((double) statsInterval / 1000) + " " + timedf.format((double) realElapsedTime / 1000000000L) + "s "
                    + df.format(timingError) + "% " + frameCount + "c " + df.format(actualFPS) + " " + df.format(averageFPS) + " afps");
