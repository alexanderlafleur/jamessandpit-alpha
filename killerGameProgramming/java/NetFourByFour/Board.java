...
public class Board {
    ...
    /*
     * playerID has selected position pos for their move. Called by NetFourByFour. Refereneces to gameOver and player have been removed.
     */
    {
        this.positions.set(pos, playerID); // change the 3D marker shown at pos
        playMove(pos, playerID); // play the move on the board
    } // end of tryPosn()

    private void playMove(int pos, int playerID)
    /*
     * Apply the playerID's move to the board, and see if it is a winner. playerID argument is new for the net version.
     */
    {
        ...
        for (int j = 0; j < numWinLines; j++) {
            ...
            if (this.winLines[line][1] != playerID && this.winLines[line][1] != UNOCCUPIED) {
                this.winLines[line][0] = -1;
            } else {
                this.winLines[line][1] = playerID; // this line belongs to this
                // player
    ...
    // call gameWon() in top-level, not showMessage()
    {
        ...
        this.fbf.gameWon(playerID, score);
    } // end of reportWinner()
