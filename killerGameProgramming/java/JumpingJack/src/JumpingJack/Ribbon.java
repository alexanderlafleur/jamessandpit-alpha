...
public class Ribbon {
    ...
    /*
     * The x-coord in the panel where the start of the image (its head) should be drawn. It can range between -width to width (exclusive), so can have
     * a value beyond the confines of the panel (0-pWidth). As xImHead varies, the on-screen ribbon will usually be a combination of its tail followed
     * by its head.
     */
    ...
    /*
     * Consider 5 cases: when xImHead == 0, draw only the im head when xImHead > 0, draw the im tail and im head, or only the im tail. when xImHead <
     * 0, draw the im tail, or the im tail and im head xImHead can range between -width to width (exclusive)
     */
    {
        if (xImHead == 0) {
            ...
            draw(g, im, 0, pWidth, width - xImHead, width - xImHead + pWidth); // im
            ...
            draw(g, im, width + xImHead, pWidth, 0, pWidth - width - xImHead); // im
            // head
        }
    } // end of display()
