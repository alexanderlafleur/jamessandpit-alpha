...
public class PointHeight {
    ...
    /*
     * Calculate the two vectors u, v that define the triangle which uses the PR triangulation line pt is ABOVE / BELOW the line PR, as recorded in
     * linePos.
     */
    {
    ...
    /*
     * Calculate the two vectors u, v that define the triangle which uses the SQ triangulation line pt is ABOVE / BELOW the line SQ, as recorded in
     * linePos.
     */
    {
        Vector3d u = new Vector3d();
    ...
    // return (x,y,z)
    {
        ...
        System.out.println(ptCh + " coord: (" + df.format(pt.x) + ", " + df.format(pt.y) + ", " + df.format(pt.z) + ")");
        return pt;
    } // end of getCoord()

    private void getFileCoords(String fnm)
    /*
     * Obtain coords and point info from fnm, with the format: num num num // P coord num num num // Q coord num num num // R coord num num num // S
     * coord num num // (x,z) of point
     */
    {
        try {
    ...
    // return (x,0,z); 0 is not used
    {
        ...
        System.out.println("Point: (" + df.format(pt.x) + ", " + df.format(pt.z) + ")");
        return pt;
    } // end of getCoord()
    ...
    {
        if (pt.x < coordS.x || pt.x > coordQ.x || pt.z < coordS.z || pt.z > coordQ.z) {
            return false;
        }
        return true;
    } // end of inRange()
    ...
    /*
     * Calculate the normal to the two vectors u and v. Calculate the distance to the origin by using the dot product on the normal and any point
     * covered by the triangle. The normal is (A, B, C) and distance is D. These define the equation for the triangle: (A * x) + (B * y) + (C * z) = D
     ...
     */
    {
        ...
        System.out.println("A: " + df.format(normal.x) + ", B: " + df.format(normal.y) + ", C: " + df.format(normal.z) + ", D: " + df.format(dist)); // Ax +
        // By +
        // Cz =
        // D
