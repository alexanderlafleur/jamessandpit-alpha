...
public class KeyBehavior extends ViewPlatformBehavior {
    ...
    // place viewpoint at (0,?,0), facing into scene
    {
        ...
        steerTG.getTransform(this.t3d); // targetTG not yet available
        this.t3d.setTranslation(startPosn); // so use steerTG
    ...
    /*
     * Calculate the next position on the floor (x,?,z). Test if it is within the floor boundaries.
     * 
     * If it is then ask Landscape to get the floor height for that (x,z).
     * 
     * Then set the y value to a change so that the viewpoint will rest on the floor at (x,z)
     */
    {
        ...
        double floorHeight = this.land.getLandHeight(nextLoc.x, nextLoc.z, this.currLandHeight);
        // Calculate the change from the current y-position.
        // Reset any offset upwards back to 0.
        double heightChg = floorHeight - this.currLandHeight - (MOVE_STEP * this.zOffset);

        this.currLandHeight = floorHeight; // update current height
