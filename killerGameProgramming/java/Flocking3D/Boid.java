...
public class Boid extends BranchGroup {
    ...
    /*
     * The boid avoids obstacles by checking it's bounding sphere against all the obstacles. The avoidance velocity is stored in avoidOb. We don't
     * bother calculating a real rebound velocity, just move away by some random amount in the x- and z- direction. We don't handle impacts on the top
     * of obstacles which require a change in the y- direction.
     */
    {
        avoidOb.set(0, 0, 0); // reset
        // update the BoundingSphere's position (but its radius stays the same)
        bs.setCenter(new Point3d(boidPos.x, boidPos.y, boidPos.z));
    ...
    /*
     * Apply the velocity rules, storing the new velocities in the velChanges ArrayList. Then add the velocities together, and limit the total speed
     * to maxSpeed. The velocity rules are only applied if there are no obstacles -- obstacle avoidance has priority over the velocity rules
     */
    {
    ...
    /*
     * Adjust the boid's position and velocity so it stays within the volume of space defined by MIN_PT and MAX_PT. Also check if perching should be
     * started.
     */
    {
        ...
        perchInterval++;
        if (perchInterval > PERCH_INTERVAL && boidPos.y <= MIN_PT.y) { // let
            // the
            // boid
            // perch
