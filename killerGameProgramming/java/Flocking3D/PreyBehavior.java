...
public class PreyBehavior extends FlockBehavior {
    ...
    /*
     * Initialise the Boidslist of PreyBoid objects and add the TransformGroups for these boids to boidsBG. Boid is a subclass of TransformGroup so
     * can be added to boidsBG directly.
     */
    { // preyBoids can be detached from the scene
        boidsBG.setCapability(Group.ALLOW_CHILDREN_WRITE);
        boidsBG.setCapability(Group.ALLOW_CHILDREN_EXTEND);
        PreyBoid pb;
    ...
    /*
     * PreyBoid 'i' is to be eaten. This causes it to be detached from the scene and removed from its BoidsList. Called by eatFood() in
     * PredatorBehavior.
     */
    {
    ...
    /*
     * If the boid is close to a predator, then it moves in the opposite direction by a scaled amount. This is an extra velocity rule, used by each
     * PreyBoid by calling doVelocityRules()
     */
    {
