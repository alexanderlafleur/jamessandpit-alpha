...
// Holds the 3D canvas where the loaded image is displayed
{
     ...
     * 
     */
    private static final long serialVersionUID = 2229314757265457573L;
    ...
    // initilise the scene
    {
        ...
        if (this.loadedScene != null) {
            showNamedObject(this.loadedScene); // display its named objects
            storeGraphInfo(this.loadedBG); // traverse model's graph; save info to
            // file

            // adjust the model's component shapes in various ways
            adjustShapes(this.loadedBG);
        }
    ...
    /*
     * Load the model from fn into the scene graph using a NCSA Portfolio loader. Rotate and scale it to make it easier to see. Store the loaded model's scene in the global
     * loadedScene, and its branch group in loadedBG.
     */
    {
        ...
        try {
            ...
            this.loadedScene = loader.load(fn); // the loaded scene

            // Rotate and scale the model
            if (this.loadedScene != null) {
                this.loadedBG = this.loadedScene.getSceneGroup(); // the model's BG

                Transform3D t3d = new Transform3D();
                t3d.rotX(-Math.PI / 2.0); // models are often on their face;
                // fix that
                Vector3d scaleVec = calcScaleFactor(this.loadedBG, fn); // scale the
                // model
                t3d.setScale(scaleVec);

                TransformGroup tg = new TransformGroup(t3d);
                tg.addChild(this.loadedBG);

                this.sceneBG.addChild(tg); // add (tg->loadedBG) to scene
            } else {
    ...
    // Scale the model based on its original bounding box size
    {
        ...
        System.out.println("max dimension: " + this.df.format(max) + "; scaleFactor: " + this.df.format(scaleFactor));
    ...
    /*
     * Display the named objects, if any. The naming scheme is file-type dependent, but includes VRML DEF names and Lightwave 3D object filenames.
     */
    {
        String name;
    ...
    /*
     * Traverse the model's scene graph and store information in EXAMINE_FN via the ofw FileWriter.
     */
    {
        System.out.println("Writing model details to " + EXAMINE_FN);
        try {
            this.ofw = new FileWriter(EXAMINE_FN);
            examineNode(0, bg);
            this.ofw.close();
        } catch (IOException ioe) {
    ...
    /*
     * A Node can be a Group or a Leaf. Depending on the type of Group or Leaf, report different things. Recursively call examineNode() on the children of a Group, incrementing
     * level so the output can be indented correctly.
     */
    {
        if (node instanceof Group) { // the Node is a Group
            ...
            if (g instanceof TransformGroup) { // consider subclass
                ...
                levelPrint(level, t3d.toString()); // show Transform3D info for
                // TG
            }
            ...
            if (node instanceof Shape3D) {
                examineShape3D(level, (Shape3D) node); // treat Shape3D
                // specially
            }
        } else {
            // the Node is something other than a Group or Leaf
    ...
    /*
     * A Shape3D is a container for Appearance and Geometry components. Show appearance info with printAppearance(). A shape may contain many geometries: examine each one with
     * examineGeometry()
     */
    {
    ...
    private void printAppearance(int level, Appearance app) throws IOException
    /*
     * Lots of Appearance information could be printed here. We only display colour related stuff.
     */
    {
    ...
    private void examineGeometry(int level, int index, Geometry geo) throws IOException
    // Display geometry info found in a shape
    {
        levelPrint(level, "Geometry: " + geo.getClass());
        if (geo instanceof GeometryArray) {
            // geometryArray is frequently used in models, so give some extra
            // info
            levelPrint(level, "Vertex count: " + ((GeometryArray) geo).getVertexCount());
        }
    ...
    private void adjustShapes(Node node) {
        ...
        if ((this.adaptNo == 3) || (this.adaptNo == 4)) {
            // shapes
    ...
    // load image from file fn as a texture
    {
        ...
        this.texture = (Texture2D) texLoader.getTexture();
    ...
    /*
     * If the node is a Group then recursively visit its children. Otherwise if the node is a Shape3D then so the changes.
     */
    {
    ...
    /*
     * Shape adjusting examples: change the colour of a shape to blue draw the shape in outline (i.e. as a wireframe) make the shape transparent add a texture to the shape (this
     * can be combined with the colour changing method) The choice of which one depends on adaptNo: 0-3, and 4 does makeBlue() and addTexture()
     */
    {
        switch (this.adaptNo) {
        case 0:
    ...
    /*
     * Add a texture to the shape, but only if it is represented by a single GeometryArray.
     */
    {
    ...
    /*
     * Add a texture to a GeometryArray. Add the texture to all faces of the model, 'stretch' the texture over the model, modulate the texture with the existing colour and lighting
     * components. The texture comes from TEXTURE_FN, and was loaded before the traversal of the model began.
     */
    {
        Appearance app = shape.getAppearance();
        ...
        if (this.texture != null) { // loaded at start, from adjustShapes()
    ...
    /*
     * Specify how the texture is mapped to the model by adjusting the genration planes so that a single texture is 'stetched' over the entire model rather than repeatedly 'tiled'.
     * 
     * This makes a big difference for models that are very large since the default tiling approach tends to lose texture detail and the display 'shimmers' since the texture is
