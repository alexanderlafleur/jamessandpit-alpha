...
public class LatheShape3D extends Shape3D {
    ...
    /*
     * Create the surface, using the curve defined by the (x,y) coords in xs[] and ys[].
     * 
     * The surface is a QuadArray, which is given normals so it will reflect light.
     * 
     * Texture coordinates may be defined to wrap the image around the outside of the shape, starting from the back, wrapping counter-clockwise (left to right) around the front,
    ...
    /*
     * Wrap the texture around the shape, the left edge starting at the back, going counter-clockwise round the front. Th texture is stretched along the y-axis so a t value of 1
     * equals the max height of the shape.
     * 
     * s is obtained from the angle made by the (x,z) coordinate; t is the scaled height of a coordinate.
     */
    {
        ...
        for (int i = 0; i < numVerts / 3; i++) {
            ...
            tVal = (float) (y / this.height); // 0.0f to 1.0f; uses global height
            // value
     ...
     * 
     * The s coords on the -PI side will be near 0, the ones on the PI side will be near 1, which will make the square show a reversed texture.
     * 
     * The correction is to change the 0 values to 1's, which will make the square show the texture near the s value of 1.
     */
    {
    ...
    /*
     * Deruved from the SurfaceOfRevolution class by Chris Buckalew, (c) 1994-2002. Part of his FreeFormDef.java example. http://www.csc.calpoly.edu/~buckalew/474Lab6-W03.html
     */
     ...
     * 
     * The coords in the xs[] and ys[] arrays are assumed to be in increasing order.
     */
    {
        checkCoords(xs);
    ...
    /*
     * ------------------------------------------ The following methods rotate the radius unchanged, creating a circle of points. These methods can be overridden to vary the radii
     * of the points, e.g. to make an ellipse.
     */
