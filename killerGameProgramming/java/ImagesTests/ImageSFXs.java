...
public class ImageSFXs {
    ...
    public static final int DOUBLE_FLIP = 2; // flip horizontally and
    // vertically
    ...
    // Create pre-defined image operations for image negation and blurring.
    {
        ...
        this.negOp = new RescaleOp(-1.0f, 255f, null);

        // image negative for images with transparency
        float[] negFactors = { -1.0f, -1.0f, -1.0f, 1.0f }; // don't change the
        // alpha
        float[] offsets = { 255f, 255f, 255f, 0.0f };
        this.negOpTrans = new RescaleOp(negFactors, offsets, null);
        ...
        this.blurOp = new ConvolveOp(new Kernel(3, 3, blurKernel), ConvolveOp.EDGE_NO_OP, null);
    ...
    /*
     * Return a new image which has been flipped horizontally, vertically, or both.
     */
    {
        if (im == null) {
        ...
        BufferedImage copy = this.gc.createCompatibleImage(imWidth, imHeight, transparency);
    ...
    // BufferedImage object.
    {
        ...
        int imWidth = im.getWidth();
        int imHeight = im.getHeight();
        g2d.drawImage(im, x + imWidth, y, x, y + imHeight, 0, 0, imWidth, imHeight, null);
    ...
    // BufferedImage object.
    {
        ...
        int imWidth = im.getWidth();
        int imHeight = im.getHeight();
        g2d.drawImage(im, x, y + imHeight, x + imWidth, y, 0, 0, imWidth, imHeight, null);
    ...
    /*
     * The degree of fading is specified with the alpha value. alpha == 1 means fully visible, 0 mean invisible.
     */
    {
        if (im == null) {
    ...
    /*
     * Create a new BufferedImage which is the input image, rotated angle degrees clockwise.
     * 
     * An issue is edge clipping. The simplest solution is to design the image with plenty of (transparent) border.
     */
    {
        if (src == null) {
    ...
    // blurring with a fixed convolution kernel
    {
        ...
        g2d.drawImage(im, this.blurOp, x, y); // use the predefined ConvolveOp
    } // end of drawBlurredImage()

    public void drawBlurredImage(Graphics2D g2d, BufferedImage im, int x, int y, int size)
    /*
     * The size argument is used to specify a size*size blur kernel, filled with 1/(size*size) values.
     * 
     * Size should be odd so that the center cell of the kernel corresponds to the coordinate being blurred.
     * 
     * The larger the size value, the larger the kernel, and more blurry the resulting image.
     * 
     * An issue is the edge effects, which will produce a nasty black border or a border with no bluriness, depending on which ConvolveOp EDGE constant is used.
     */
    {
        if (im == null) {
            System.out.println("getBlurredImage: input image is null");
        ...
        ConvolveOp blurringOp = new ConvolveOp(new Kernel(size, size, blurKernel), ConvolveOp.EDGE_NO_OP, null); // leaves
        // edges
        // unaffected
    ...
    /*
     * There are two versions of drawRedderImage() here: one using a LookupOp, the other a RescaleOp. There is functionally no difference between them.
     */

    public void drawRedderImage(Graphics2D g2d, BufferedImage im, int x, int y, float brightness)
    // using LookupOp
    /*
     * Draw the image with its redness is increased, and its greenness and blueness decreased. Any alpha channel is left unchanged.
     */
    {
        if (im == null) {
    ...
    /*
     * public void drawRedderImage(Graphics2D g2d, BufferedImage im, int x, int y, float brightness) // using RescaleOp // Draw the image with its redness is increased, and its
     * greenness // and blueness decreased. Any alpha channel is left unchanged. { if (im == null) { System.out.println("drawRedderImage: input image is null"); return; }
     * 
     * if (brightness < 0.0f) { System.out.println("Brightness must be >= 0.0f; setting to 0.0f"); brightness = 0.0f; } // brightness may be less than 1.0 to make the image less
     * red
     * 
     * RescaleOp brigherOp; if (hasAlpha(im)) { float[] scaleFactors = {brightness, 1.0f/brightness, 1.0f/brightness, 1.0f}; // don't change alpha // without the 1.0f the RescaleOp
     * fails, which is a bug (?) float[] offsets = {64.0f, 0.0f, 0.0f, 0.0f}; brigherOp = new RescaleOp(scaleFactors, offsets, null); } else { // not transparent float[]
     * scaleFactors = {brightness, 1.0f/brightness, 1.0f/brightness}; float[] offsets = {64.0f, 0.0f, 0.0f}; brigherOp = new RescaleOp(scaleFactors, offsets, null); }
     * g2d.drawImage(im, brigherOp, x, y); } // end of drawRedderImage() using RescaleOp
     */
    ...
    /*
     * Draw the image with changed brightness, by using a RescaleOp. Any alpha channel is unaffected.
     */
    {
        if (im == null) {
    ...
    /*
     * Draw the image with 255-<colour value> applied to its RGB components. Any alpha channel is unaffected.
     */
    {
        if (im == null) {
        ...
        if (hasAlpha(im)) {
            g2d.drawImage(im, this.negOpTrans, x, y); // use predefined RescaleOp
        } else {
            g2d.drawImage(im, this.negOp, x, y);
        }
    ...
    /* Mix up the colours in the green and blue bands of the image. */
    {
        ...
        if (hasAlpha(im)) {
            float[][] colourMatrix = { { 1.0f, 0.0f, 0.0f, 0.0f }, // for new
                    // red band,
                    // unchanged
                    { r.nextFloat(), r.nextFloat(), r.nextFloat(), 0.0f }, // new
                    // green
                    // band
                    { r.nextFloat(), r.nextFloat(), r.nextFloat(), 0.0f }, // new
                    // blue
            ...
            float[][] colourMatrix = { { 1.0f, 0.0f, 0.0f }, // for new red
                    // band,
                    // unchanged
                    { r.nextFloat(), r.nextFloat(), r.nextFloat() }, // new
                    // green
                    // band
                    { r.nextFloat(), r.nextFloat(), r.nextFloat() } }; // new
            // blue
        ...
        BufferedImage newIm = new BufferedImage(im.getColorModel(), destRaster, false, null);
    ...
    /*
     * Change some of the image's pixels to 0's, which will become transparent in an image with an alpha channel, black otherwise.
     */
    {
        if (im == null) {
    ...
    // make a copy of the BufferedImage src
    {
        ...
        int transparency = src.getColorModel().getTransparency();
        BufferedImage copy = this.gc.createCompatibleImage(src.getWidth(), src.getHeight(), transparency);
        Graphics2D g2d = copy.createGraphics();
    ...
    /*
     * Make a copy of src, but in a BufferedImage object with an alpha channel.
     */
    {
        if (src == null) {
        ...
        BufferedImage dest = new BufferedImage(src.getWidth(), src.getHeight(), BufferedImage.TYPE_INT_ARGB); // alpha
        // channel
        Graphics2D g2d = dest.createGraphics();
