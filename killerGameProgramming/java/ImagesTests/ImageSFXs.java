...
public class ImageSFXs {
    ...
    // make a copy of the BufferedImage src
    {
        ...
        BufferedImage copy = gc.createCompatibleImage(src.getWidth(), src.getHeight(), transparency);
    ...
    /*
     * The size argument is used to specify a size*size blur kernel, filled with 1/(size*size) values. Size should be odd so that the center cell of
     * the kernel corresponds to the coordinate being blurred. The larger the size value, the larger the kernel, and more blurry the resulting image.
     * An issue is the edge effects, which will produce a nasty black border or a border with no bluriness, depending on which ConvolveOp EDGE
     * constant is used.
     */
    {
        if (im == null) {
    ...
    /*
     * There are two versions of drawRedderImage() here: one using a LookupOp, the other a RescaleOp. There is functionally no difference between
     * them.
     */
    ...
    /*
     * public void drawRedderImage(Graphics2D g2d, BufferedImage im, int x, int y, float brightness) // using RescaleOp // Draw the image with its
     * redness is increased, and its greenness // and blueness decreased. Any alpha channel is left unchanged. { if (im == null) {
     * System.out.println("drawRedderImage: input image is null"); return; } if (brightness < 0.0f) { System.out.println("Brightness must be >= 0.0f;
     * setting to 0.0f"); brightness = 0.0f; } // brightness may be less than 1.0 to make the image less red RescaleOp brigherOp; if (hasAlpha(im)) {
     * float[] scaleFactors = {brightness, 1.0f/brightness, 1.0f/brightness, 1.0f}; // don't change alpha // without the 1.0f the RescaleOp fails,
     * which is a bug (?) float[] offsets = {64.0f, 0.0f, 0.0f, 0.0f}; brigherOp = new RescaleOp(scaleFactors, offsets, null); } else { // not
     * transparent float[] scaleFactors = {brightness, 1.0f/brightness, 1.0f/brightness}; float[] offsets = {64.0f, 0.0f, 0.0f}; brigherOp = new
     * RescaleOp(scaleFactors, offsets, null); } g2d.drawImage(im, brigherOp, x, y); } // end of drawRedderImage() using RescaleOp
     */
    ...
    /*
     * Create a new BufferedImage which is the input image, rotated angle degrees clockwise. An issue is edge clipping. The simplest solution is to
     * design the image with plenty of (transparent) border.
     */
    {
        if (src == null) {
