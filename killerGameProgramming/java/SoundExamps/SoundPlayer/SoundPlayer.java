...
public class SoundPlayer extends JFrame implements ActionListener {
    ...
    public SoundPlayer() {
        ...
        this.playingClips = new ArrayList();
        initGUI();
        loadSounds();
     ...
     */
    {
        ...
        this.playListJcb = new JComboBox(soundFNms);
     ...
     */
    {
        ...
        for (String element : soundFNms) {
            ...
            if (clip == null) {
                ...
                this.soundsMap.put(element, clip);
            }
        }
    } // end of loadSounds()
    ...
    /*
     * Retrieve the relevant AudioClip, and start it playing or looping. Store a reference to the clip in the playingClips ArrayList since it is playing.
     */
    {
        String chosenFile = (String) this.playListJcb.getSelectedItem();

        // try to get the AudioClip.
        AudioClip audioClip = (AudioClip) this.soundsMap.get(chosenFile);
        if (audioClip == null) {
        ...
        this.playingClips.add(audioClip); // store a reference to the playing clip
    ...
    /*
     * Stop all the music by calling stop() on all the references in playingClips. The issue is that some of the clips may have already finished playing by themselves, but there is
     * no way to detect that when using the AudioClip class.
     */
    {
