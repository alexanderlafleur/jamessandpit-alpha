...
public class EchoSamplesPlayer {
    ...
    {
        if (((format.getEncoding() == AudioFormat.Encoding.PCM_UNSIGNED) || (format.getEncoding() == AudioFormat.Encoding.PCM_SIGNED)) && (format.getSampleSizeInBits() == 8)) {
            return true;
        } else {
            return false;
        }
    ...
    /*
     * Load all the samples from the AudioInputStream as a single byte array. Return a _modified_ byte array, after the echo effect has been applied.
     */
    {
        ...
        /*
         * Create a byte array by applying the audio effect. This line is the main point of difference from SamplesPlayer, which returns samples unchanged.
         */
    ...
    /*
     * The echo effect is ECHO_NUMBER (4) copies of the original sound added to the end of the original.
     * 
     * The volume of each one is reduced (decayed) by DECAY (0.5) over its predecessor.
     * 
    ...
    /*
     * Since the effect is restricted to samples which are PCM signed or unsigned, and 8-bit, we do not have to worry about big/little endian or strange byte formats. The byte
     * represents the amplitude (loudness) of the sample.
     * 
     * The byte is converted to a short, divided by the decay, then returned as a byte.
     * 
     * An unsigned byte needs masking as it is converted since Java stores shorts in signed form, so we cut away any excessive bits before the short is created (which uses 16
     * bits).
     * 
     */
    {
    ...
    /*
     * The samples[] byte array is connected to a stream, read in chunks, and passed to the SourceDataLine.
     */
    {
        // byte array --> stream
