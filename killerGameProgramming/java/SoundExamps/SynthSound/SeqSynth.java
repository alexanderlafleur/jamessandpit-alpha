...
public class SeqSynth implements MetaEventListener {
    ...
    /*
     * The note offsets use the "C" major scale, which has the order "C D E F G A B", but the offsets are stored in the order "A B C D E F G" to simplify their lookup.
     */
    ...
    /*
     * The sequence uses PPQ (pulses per quarter (ticks per beat) The actual number of pulses is defined using resolution. The higher the resolution, the faster (greater tempo) to
     * the music.
     * 
    ...
    /*
     * Adding a note is really the addition of two MidiEvent messages to the track: a NOTE_ON to start the note playing, and a NOTE_OFF to stop it. The playing length is determined
     * by the time interval (period) separating the time-stamps in the two messages.
     */
    {
        setMessage(ShortMessage.NOTE_ON, note, this.tickPos);
        this.tickPos += period;
        setMessage(ShortMessage.NOTE_OFF, note, this.tickPos);
    }
    ...
    /*
     * An instrument is defined by its 'bank' and 'program' integers. If the supplied bank and program match an actual instrument, then update the synthesizer's instrument
     * settings.
     */
    {
        Instrument[] instrument = this.synthesizer.getAvailableInstruments();
     ...
     */
    {
        ...
        try {
            ...
            MidiEvent event = new MidiEvent(message, this.tickPos);
            this.track.add(event);
        } catch (InvalidMidiDataException e) {
            e.printStackTrace();
        }
    ...
    /*
     * Start the sequence playing. The tempo setting is in BPM (beats per minute), which is combined with the PPQ (ticks / beat) resolution to determine the speed of playing.
     */
    {
        try {
