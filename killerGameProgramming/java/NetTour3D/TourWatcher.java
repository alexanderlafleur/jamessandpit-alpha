...
public class TourWatcher extends Thread {
    ...
    public TourWatcher(WrapNetTour3D w3d, BufferedReader i, Obstacles obs) {
        this.w3d = w3d;
        this.in = i;
        this.visitors = new HashMap(); // no visitors yet
    }
    ...
    /*
     * Create a visitor to the world on this machine. Message format: create <name> xPosn zPosn
     * 
     * The visitor has the name <name>, and their sprite will be located at (xPosn ,zPosn). The sprite will be the robot, oriented along the +z-axis.
     * 
     ...
     */
    {
        ...
        if (this.visitors.containsKey(userName)) {
            System.out.println("Duplicate name -- ignoring it"); // should
        } else {
            System.out.println("Creating Sprite");
            DistTourSprite dtSprite = this.w3d.addVisitor(userName, xPosn, zPosn, 0);
            this.visitors.put(userName, dtSprite);
        }
    } // end of createVisitor()

    private void sendDetails(String line)
    /*
     * Msg format: wantDetails cliAddr port The client at (cliAddr, port) wants info about this client. The reason is to render the client as a sprite.
     * 
     * Ask WrapNetTour3D (w3d) to send the details in a 'detailsFor' message.
     */
    {
        StringTokenizer st = new StringTokenizer(line);
        ...
        this.w3d.sendDetails(cliAddr, strPort);
    } // end of sendDetails()

    private void receiveDetails(String line)
    /*
     * Another client sends details about itself to this client. The reason is to render the other client here on this machine.
     * 
     * Message format: detailsFor userName xPosn zPosn rotRadians
     * 
     * (xPosn,zPosn) is the position of the sprite, with rotRadians its rotation away from facing front.
     * 
     ...
     */
    {
        ...
        if (this.visitors.containsKey(userName)) {
            ...
            DistTourSprite dtSprite = this.w3d.addVisitor(userName, xPosn, zPosn, rotRadians);
            this.visitors.put(userName, dtSprite);
        }
    ...
    /*
     * Message formats: <name> <command> where <command> can be one of: forward | back | left | right | rotCClock | rotClock | bye
     * 
     * Most of these commands are to duplicate the movement of a client's sprite in the copy here.
     * 
     ...
     */
    {
        ...
        DistTourSprite dtSprite = (DistTourSprite) this.visitors.get(userName);
        if (dtSprite == null) {
