...
public class ImagesSeries extends Shape3D {
    ...
    private void createAppearance() {
        ...
        /*
         * // mix the texture and the material colour TextureAttributes ta = new TextureAttributes(); ta.setTextureMode(TextureAttributes.MODULATE);
         * app.setTextureAttributes(ta); Material mat = new Material(); // set material and lighting mat.setLightingEnable(true);
         * app.setMaterial(mat);
         */
        // Create a two dimensional texture
        // Set the texture from the first loaded image
        texture = new Texture2D(Texture.BASE_LEVEL, Texture.RGBA, ims[0].getWidth(), ims[0].getHeight());
        texture.setImage(0, ims[0]);
        texture.setCapability(Texture.ALLOW_IMAGE_WRITE); // texture can
        // change
    ...
    public void showSeries() {
        for (ImageComponent2D element : ims) {
            texture.setImage(0, element);
            try {
