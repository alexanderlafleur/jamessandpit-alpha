...
public class MultiChat extends JFrame implements ActionListener {
    ...
    /*
     * synchronized private void showMsg(String msg) // Synchronized since this method can be called by the GUI // and application threads. {
     * jtaMesgs.append(msg); }
     */
    ...
    /*
     * Extract details from the received packet. Format: (fromName): msg [ / toName ] A "/ toName" extension means that the message will only appear
     * in the sender's text window and toName's window.
     */
    {
     ...
     */
    {
        ...
        try {
            DatagramPacket sendPacket = new DatagramPacket(labelledMsg.getBytes(), labelledMsg.length(), groupAddr, GROUP_PORT);
    ...
    {
        try {
            DatagramPacket sendPacket = new DatagramPacket(msg.getBytes(), msg.length(), serverAddr, SERVER_PORT);
     ...
     * invokeLater(). msg must be final to be used inside the inner class for Runnable. Thanks to Rachel Struthers (rmstruthers@mn.rr.com)
     */
    {
    ...
    /*
     * Repeatedly receive a packet from the group, process it. No messages are sent from here. Output is done from the GUI thread by calling
     * sendPacket().
     */
    {
