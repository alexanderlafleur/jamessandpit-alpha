...
public class ChatServlet extends HttpServlet {
     ...
     * 
     * The client wants to join the chat group. The server returns a uid cookie, or rejects the client.
     */
    {
     ...
     * 
     * The client is departing. use the name and cookie to delete their details.
     */
    {
    ...
    /*
     * Message format: ChatServlet?cmd=who Return a list of who is logged on currently. There is no need for the client to send a name or cookie.
     */
    {
        PrintWriter output = response.getWriter();
    ...
    private void processMsg(HttpServletRequest request, HttpServletResponse response) throws IOException
    /*
     * Message format: ChatServlet?cmd=msg&name=??&msg=?? + uid cookie Add the message to the server's list
     */
    {
     ...
     * 
     * All the visible messages stored by the server since the last read by this user are sent back to him.
     */
    {
        int uid = -1; // default for failure
        String userName = request.getParameter("name");
