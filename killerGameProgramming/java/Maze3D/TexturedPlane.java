...
public class TexturedPlane extends Shape3D {
    ...
    public TexturedPlane(ArrayList coords, String fnm, Vector3f normal) {
        ...
        this.plane = new QuadArray(this.numPoints, GeometryArray.COORDINATES | GeometryArray.TEXTURE_COORDINATE_2 | GeometryArray.NORMALS);
    ...
    private void createGeometry(ArrayList coords, Vector3f normal) {
        // set coordinates
        Point3f[] points = new Point3f[this.numPoints];
        coords.toArray(points);
        this.plane.setCoordinates(0, points);

        // assign texture coords to each quad
        // counter-clockwise, from bottom left
        TexCoord2f[] tcoords = new TexCoord2f[this.numPoints];
        for (int i = 0; i < this.numPoints; i = i + 4) {
        ...
        this.plane.setTextureCoordinates(0, 0, tcoords);

        // all the coords are given the same normal since they are in a plane
        for (int i = 0; i < this.numPoints; i++) {
            this.plane.setNormal(i, normal);
        }
