...
public class WrapMaze3D extends JPanel {
    ...
    {
        this.mazeMan = mm; // ref to maze manager
        this.camera2TG = c2TG; // ref to second back-facong camera
    ...
    // initilise the scene
    {
        ...
        this.sceneBG.addChild(floor.getBG());

        this.sceneBG.addChild(this.mazeMan.getMaze()); // add maze, using MazeManager
        this.sceneBG.addChild(this.camera2TG); // add second camera
    ...
    /*
     * private void addBackground() // add a geometric background using a Background node { System.out.println("Loading sky texture: " + SKY_TEX); TextureLoader tex = new
     * TextureLoader(SKY_TEX, null);
     * 
     * Sphere sphere = new Sphere(1.0f, Sphere.GENERATE_NORMALS | Sphere.GENERATE_NORMALS_INWARD | Sphere.GENERATE_TEXTURE_COORDS, 4); // default = 15 Appearance backApp =
     * sphere.getAppearance(); backApp.setTexture( tex.getTexture() );
     * 
     ...
     * 
     * sceneBG.addChild(bg); } // end of addBackground()
     */
    ...
    private void prepareViewPoint(BirdsEye be) {
        ...
        View userView = this.su.getViewer().getView();
        ...
        KeyBehavior keybeh = new KeyBehavior(this.mazeMan, be, this.camera2TG);
    ...
    // rotate and move the viewpoint
    {
        ...
        t3d.mul(toRot);
        t3d.setTranslation(this.mazeMan.getMazeStartPosn()); // place at maze start
    ...
    // a spotlight to help the user see in the (relative) darkness
    {
        ...
        spot.setInfluencingBounds(this.bounds);
        return spot;
    } // end of makeSpot()
