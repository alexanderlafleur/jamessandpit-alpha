...
public class MazeManager {
    ...
    /*
     * The coords are maze[Z]{X] i.e. x-coords along row, z-coords running down Remember that the maze plan's y-coords have been mapped to 3D scene z-coords.
     */
    ...
    private void initialiseVars() {
        ...
        this.xStartPosn = LEN / 2; // default position (top row, in the middle)
        this.zStartPosn = 0;

        // initialise the two maze representations
        this.mazeBG = new BranchGroup();
        this.mazeImg = new BufferedImage(IMAGE_LEN, IMAGE_LEN, BufferedImage.TYPE_INT_ARGB);

        // the Appearance nodes used by all the blocks and cylinders
        this.blockApp = makeApp(blue, BLOCK_TEX); // blue texture for blocks
        this.cylApp = makeApp(medgreen, CYL_TEX); // green texture for cylinders
    ...
    // Initialise maze[][] by reading the maze plan from fn
    {
        ...
        try {
            ...
            while ((numRows < LEN) && ((line = br.readLine()) != null)) {
                ...
                int x = 0;
                while ((x < LEN) && (x < charLine.length)) { // ignore any
                    // extra chars
                    this.maze[numRows][x] = charLine[x];
                    x++;
                }
                numRows++;
            }
     ...
     * 
     * The maze plan array will contain spaces, a single 's' (starting point), and many 'c's and 'b's (cylinders, blocks).
     */
    {
        System.out.println("Building maze representations... please wait");

        char ch;
        Graphics g = this.mazeImg.createGraphics();
        ...
        for (int z = 0; z < LEN; z++) {
            for (int x = 0; x < LEN; x++) {
                ...
                if (ch == 's') { // starting position
                    this.xStartPosn = x;
                    this.zStartPosn = z;
                    this.maze[z][x] = ' '; // clear cell
                } else if (ch == 'b') { // block
                    this.mazeBG.addChild(makeObs(ch, x, z, this.blockApp));
                    drawBlock(g, x, z);
                } else if (ch == 'c') { // cylinder
                    this.mazeBG.addChild(makeObs(ch, x, z, this.cylApp));
                    drawCylinder(g, x, z);
                }
            }
        }
    ...
    // place an obstacle (block/cylinder) at (x,z) with appearance app
    {
        Primitive obs;
        if (ch == 'b') {
            obs = new Box(RADIUS, HEIGHT / 2, RADIUS, Primitive.GENERATE_TEXTURE_COORDS | Primitive.GENERATE_NORMALS, app);
        } else {
            // green textured cylinder
            obs = new Cylinder(RADIUS, HEIGHT, Primitive.GENERATE_TEXTURE_COORDS | Primitive.GENERATE_NORMALS, app);
        }
    ...
    {
        return new Vector3d(this.xStartPosn, USER_HEIGHT, this.zStartPosn);
    }
    ...
    // called by BirdsEye
    {
        return new Point(this.xStartPosn * IMAGE_STEP, this.zStartPosn * IMAGE_STEP);
    }
    ...
    // Called by the KeyBehavior object to test a possible move.
    {
        ...
        if ((this.maze[z][x] == 'b') || (this.maze[z][x] == 'c')) {
